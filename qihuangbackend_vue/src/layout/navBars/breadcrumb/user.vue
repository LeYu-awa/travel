...
export default {
  ...
  mounted() {
    ...
    if (this.is_area == 0) {
      ...
      this.regions = this.getRegions(JSON.parse(regions));
      let value = localStorage.getItem("regionsValue");
      if (value) {
        this.address = JSON.parse(value);
      }
    }
    // getBaseInfo()
    //   .then(res => {
  ...
  methods: {
    getRegions(data) {
      ...
      const regions = data.map(item => {
        const [province, city, district] = item.region_name.split(",");
        return {
          province,
          city: city || null, // 二级地区（市）可能没有区
          district: district || null, // 三级地区（区）
          item // 原始数据（可选，用于保留id等信息）
        };
      });

      // 2. 构建嵌套结构并排序
      const result = [];
      const provinceMap = {}; // 临时存储省份，key为省份名称
      ...
      regions.forEach(region => {
        ...
        if (city) {
          // 查找当前省下的市是否已存在
          let cityNode = provinceNode.children.find(c => c.name === city);
          if (!cityNode) {
            cityNode = { name: city, children: [] };
            provinceNode.children.push(cityNode);
            // 对市按名称排序
            provinceNode.children.sort((a, b) => a.name.localeCompare(b.name));
          }

          // 处理区（三级）
          if (district) {
            const districtNode = { name: district };
            cityNode.children.push(districtNode);
            // 对区按名称排序
            cityNode.children.sort((a, b) => a.name.localeCompare(b.name));
          }
        }
      });
    ...
    selectRegions(e) {
      if (e[0] == "全部") {
        localStorage.removeItem("regionsValue");
      } else {
        localStorage.setItem("regionsValue", JSON.stringify(e));
      }
    ...
    getTodoList() {
      needDealtList()
        .then(res => {
          this.list = res.data;
          this.dealtList =
            res.data.length > 3 ? res.data.slice(0, 3) : res.data;
        })
        .catch(res => {});
    },
    // 全屏点击
