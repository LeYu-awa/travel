...
export default {
  ...
  methods: {
    ...
    setColumnsAsideMove(k) {
      ...
      this.liIndex = k;
      this.$refs.columnsAsideActiveRef.style.top = `${els[k].offsetTop +
        this.difference}px`;
    },
    // 菜单高亮点击事件
    ...
    setFilterRoutes() {
      ...
      this.columnsAsideList = this.filterRoutesFun(
        this.$store.state.user.menuList
      );
      //   const resData = getHeaderName(this.$route.path, this.columnsAsideList);
      const resData = this.setSendChildren(
        getHeaderName(this.$route, this.columnsAsideList)
      );
      if (!resData.children) {
    ...
    setColumnsMenuHighlight(path) {
      ...
      const currentSplitRoute = this.columnsAsideList.find(
        v => v.path === path
      );
  ...
  watch: {
    ...
    "$store.state": {
      handler(val) {
        val.themeConfig.themeConfig.columnsAsideStyle === "columnsRound"
          ? (this.difference = 3)
          : (this.difference = 0);
        if (val.user.menuList.length === this.columnsAsideList.length)
          return false;
      },
      deep: true
    },
    ...
    $route: {
      handler(to) {
        ...
        let asideList = getMenuSider(this.columnsAsideList, HeadName)[0]
          .children;
