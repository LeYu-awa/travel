    ...
    <el-card class="mt14">
      ...
      >
        ...
        <el-table-column label="操作" min-width="200" fixed="right">
          <template slot-scope="scope">
            <el-button type="text" size="small" @click="onPassword(scope.row)"
              >修改密码</el-button
            >
            ...
            <el-button
              type="text"
              size="small"
              @click="handleDelete(scope.row.area_id, scope.$index)"
              >删除</el-button
            >
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    // 添加
    onAdd() {
      // this.$modalForm(areaAdminAdd()).then(() => this.getList());
      this.editshow = true;
      this.fromData = {};
    },
    // 编辑
    onEdit(row) {
      // this.$modalForm(areaAdminEdit(id)).then(() => this.getList());
      this.fromData = { ...row };
      this.editshow = true;
    },
    // 修改密码表单
    onPassword(row) {
      this.fromData = { ...row };
      // this.$modalForm(areaAdminChangePwd(id));
      this.showPwd = true;
    },
    // 删除
    handleDelete(id, idx) {
