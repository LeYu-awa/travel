  ...
  <div class="divBox">
    ...
    <el-card class="mt14">
      ...
      <el-table v-loading="listLoading" :data="tableData.data" size="small">
        ...
        <el-table-column label="作者" prop="author.nickname" min-width="100" />
        <el-table-column
          :label="tableFrom.is_type == 1 ? '图文封面' : '视频封面图'"
          min-width="210"
        >
        ...
        <el-table-column label="推荐级别" min-width="150">
          <template slot-scope="scope">
            <el-rate disabled v-model="scope.row.start" :colors="colors">
            </el-rate>
          </template>
        </el-table-column>
        <el-table-column
          ...
          min-width="100"
        />
        <el-table-column prop="watch_cost" label="收费价格" min-width="100" />
        <el-table-column prop="sham_start" label="虚拟点赞数" min-width="100" />
        ...
        <el-table-column label="操作" min-width="200" fixed="right">
          <template slot-scope="scope">
            <el-button
              type="text"
              size="small"
              @click="onDetail(scope.row.community_id)"
              >详情</el-button
            >
            <el-button type="text" size="small" @click="onEditNumber(scope.row)"
              >虚拟点赞数/虚拟评论数</el-button
            >
            <el-button
              v-if="scope.row.status == 0 || scope.row.status == -2"
              type="text"
              size="small"
              @click="onAudit(scope.row.community_id)"
              >审核</el-button
            >
            <el-button
              type="text"
              size="small"
              @click="onEdit(scope.row.community_id)"
              >编辑星级</el-button
            >
            <el-button
              v-if="scope.row.status == 1"
              type="text"
              size="small"
              @click="onOff(scope.row.community_id)"
              >强制下架</el-button
            >
            <el-button
              type="text"
              size="small"
              @click="onReply(scope.row.community_id)"
              >查看评论</el-button
            >
            <el-button
              type="text"
              size="small"
              @click="handleDelete(scope.row.community_id, scope.$index)"
              >删除</el-button
            >
          </template>
        </el-table-column>
    ...
    >
      <div v-loading="loading">
        ...
        >
          ...
          <el-form-item
            v-if="ruleForm.price === 1"
            label="收费价格"
            prop="watch_cost"
          >
            <el-input
              v-model="ruleForm.watch_cost"
              type="number"
              placeholder="请输入价格"
            />
          </el-form-item>
        ...
        <div class="box-container">
          ...
          <div class="acea-row">
            ...
            <div v-if="formData.status == 1" class="list sp100">
              ...
              <div
                v-for="(item, index) in formData.relevance"
                :key="index"
    ...
    >
      <div v-if="showVideo" class="video-container">
        <video
          style="width:600px;max-height: 500px;border-radius:10px;"
  ...
  methods: {
    ...
    onSubmit() {
      if (this.ruleForm.price === 1) {
        if (!this.ruleForm.watch_cost) {
          return this.$message.error("请填写观看费用");
        }
      } else {
        this.ruleForm.watch_cost = 0;
      }
      communityAuditApi(this.community_id, this.ruleForm)
        .then(res => {
          this.$message.success(res.message);
          this.examineVisible = false;
          this.ruleForm.watch_cost = "";
    ...
    onchangeIsShow(row) {
      communityStatusApi(row.community_id, row.is_show)
        .then(({ message }) => {
          this.$message.success(message);
        })
        .catch(({ message }) => {
