...
export default {
  ...
  methods: {
    getInfo() {
      ...
      getReceiptApi('sys_receipt_agree').then(res => {
        const info = res.data
        this.formValidate = {
          agree: info.sys_receipt_agree,
        }
    ...
    handleSubmit(name) {
      if(this.formValidate.agree === '' || !this.formValidate.agree){
        ...
        updateReceiptApi('sys_receipt_agree', this.formValidate).then(async res => {
          this.fullscreenLoading = false
