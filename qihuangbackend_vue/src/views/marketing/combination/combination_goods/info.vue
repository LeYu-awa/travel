        ...
        <span class="sp100">
          ...
          <template v-if="projectData.spec_type === 0">
            <el-table :data="OneattrValue" border class="tabNumWidth" size="small">
              ...
              <el-table-column v-for="(item,iii) in attrValue" :key="iii" :label="formThead[iii].title" align="center" min-width="90">
                <template slot-scope="scope">
          ...
          <template v-if="projectData.spec_type === 1">
            <el-table :data="ManyAttrValue" border class="tabNumWidth manyAttr" size="small">
...
<script>
import { combinationReviewDetailApi, combinationProductStatusApi } from '@/api/marketing'
const defaultObj = {
  image: '',
  slider_image: [],
  store_name: '',
...
export default {
  ...
  methods: {
    onSubmit() {
      this.ruleForm.id = this.projectData.product_group_id
      combinationProductStatusApi(this.ruleForm).then(res => {
    ...
    getInfo(id) {
      this.proId = id
      this.loading = true
      combinationReviewDetailApi(id).then(res => {
        this.projectData = res.data
        if (this.projectData.spec_type === 0) {
          this.OneattrValue = res.data.attrValue
          if(this.OneattrValue.length > 0){
            this.OneattrValue[0]['price'] = this.OneattrValue[0]['_sku']['active_price']
            this.OneattrValue[0]['stock'] = this.OneattrValue[0]['_sku']['stock']
          }
        } else {
          this.ManyAttrValue = []
          res.data.attrValue.forEach((o,i)=>{
            if(o._sku){
              ...
              this.ManyAttrValue[i]['price'] = this.ManyAttrValue[i]['_sku']['active_price']
              this.ManyAttrValue[i]['stock'] = this.ManyAttrValue[i]['_sku']['stock']
            }
          })
        }
        const tmp = {}
