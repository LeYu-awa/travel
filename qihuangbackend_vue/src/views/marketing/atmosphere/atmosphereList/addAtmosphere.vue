      ...
      >
        <div v-show="currentTab == 1">
          <el-form-item label="活动名称" prop="activity_name">
            <el-input v-model="formValidate.activity_name" size="small" class="selWidth" placeholder="请输入活动名称" />
          </el-form-item>
          ...
          <el-form-item label="活动氛围图：">
            ...
            <p class="desc mt10">宽750px，高100px
              <el-popover
                placement="bottom"
                title=""
                min-width="200"
                trigger="hover"
                >
                <img :src="`${baseURL}/statics/system/activityFrame.png`" style="height:270px;" alt="">
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
       if (valid) {
          ...
          this.isEdit
            ? atuosphereUpdateApi(this.$route.params.id, this.formValidate)
                .then((res) => {
                  this.$message.success(res.message);
                  this.$router.push({ path: this.roterPre + '/marketing/atmosphere/list' });
                })
                .catch((res) => {
                  this.$message.error(res.message);
                })
            : createAtuosphere(this.formValidate)
                .then((res) => {
                  this.$message.success(res.message);
                  this.$router.push({ path: this.roterPre + '/marketing/atmosphere/list' });
                })
