...
import { healthVideoMemberCardList, healthVideoMemberCardDelete, healthVideoMemberCardEdit } from '@/api/marketing'
...
export default {
  ...
  data() {
    return {
      ...
      tableHeaderData: [
        ...
        { prop: 'card_type_text', label: '卡包', columnType: 'text', align: 'left', 'min-width': '100' },
        { prop: 'validity_days', label: '有效期', columnType: 'text', align: 'left', 'min-width': '249' },
        ...
        { prop: 'original_price', label: '原价', columnType: 'text', align: 'left', 'min-width': '100' },
        { prop: 'stock', label: '库存', columnType: 'text', align: 'left', 'min-width': '100' },
        { prop: 'sold_count', label: '销量', columnType: 'text', align: 'left', 'min-width': '100' },
        { prop: 'description', label: '详情', columnType: 'text', align: 'left', 'min-width': '249' },
        { prop: 'benefits', label: '权益说明', columnType: 'text', align: 'left', 'min-width': '249' },
        { prop: 'thumbnail_url', label: '封面图', columnType: 'image', align: 'left', 'min-width': '100' },
        { prop: 'banner_images', label: '轮播图', columnType: 'image', align: 'left', 'min-width': '100' },
        {
          prop: 'status', label: '状态', columnType: "switch",
          activeText: '上架',
          inactiveText: '下架', align: 'left', 'min-width': '100'
        },
        {
          prop: 'is_hot', label: '是否热门', columnType: "switch",
          activeText: '是',
          inactiveText: '否', align: 'left', 'min-width': '100'
        },
        {
          prop: 'is_recommend', label: '是否推荐', columnType: "switch",
          activeText: '是',
          inactiveText: '否', align: 'left', 'min-width': '100'
        },
        { prop: 'validity_text', label: '有效时间', columnType: 'text', align: 'left', 'min-width': '249' },
  ...
  methods: {
      ...
      switch (name) {
        ...
        case 'del':
          this.$confirm('是否删除此数据?', '提示', {
            ...
            this.IfhealthVideoMemberCardDelete(row.id)
    ...
    // 删除接口
    IfhealthVideoMemberCardDelete(str) {
      healthVideoMemberCardDelete(str).then(res => {
        this.$message({
          type: 'success',
          message: '删除成功!'
        });
    ...
    getList(num) {
      ...
      healthVideoMemberCardList(this.tableFrom)
        .then((res) => {
