...
import { healthSymptomList,healthSymptomDelete, healthSymptomEdit } from '@/api/marketing'
...
export default {
  ...
  data() {
    return {
      ...
      tableData: [],
      tableHeaderData: [
        { prop: 'name', label: '头痛', columnType: 'text', align: 'left', 'min-width': '249' },
        { prop: 'category_name', label: '身体部位', columnType: 'text', align: 'left', 'min-width': '249' },
        
        { prop: 'sort', label: '排序', columnType: 'text', align: 'left', 'min-width': '120' },
  ...
  methods: {
    ...
    onErtion(name, row) {
      switch (name) {
        case 'edit':
          this.fromData = { ...row, category: Number(row.category) }
          this.editshow = true
          break
        case 'del':
          this.$confirm('是否删除此数据?', '提示', {
              ...
              this.IfhealthSymptomDelete(row.id)
    ...
    // 删除接口
    IfhealthSymptomDelete(str) {
      healthSymptomDelete(str).then(res => {
        this.$message({
          type: 'success',
          message: '删除成功!'
        });
    ...
    getList(num) {
      ...
      healthSymptomList(this.tableFrom)
        .then((res) => {
