...
export default {
      ...
      FormData: [
        {
          ...
          validate: [{ required: true, message: "请输入分类名称", trigger: "blur" }]
        },
        {
          type: 'frame',
          field: 'logo',
  ...
  methods: {
    ...
    onSubmitForm() {
      this.fApi.submit((formData, fApi) => {
        if (formData.id) {
          ...
          this.IfaddaddLiveType(formData);
        }
      });
    },
    cancel() {
    ...
    IfaddaddLiveType(data) {
      addLiveType(data).then(res => {
        this.$emit("confirm");
      });
    },
    IfupdateEditGift(data) {
      editLiveType(data).then(res => {
        this.$emit("confirm");
      });
    }
  }
