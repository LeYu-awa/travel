...
import { getAgreementApi, updateAgreementApi } from "@/api/user";
export default {
  name: "UserAgreement",
  ...
  methods: {
    getInfo() {
      ...
      getAgreementApi("sys_coupon_agree")
        .then((res) => {
          const info = res.data;
          this.formValidate = {
            agree: info.sys_coupon_agree,
          };
    ...
    handleSubmit(name) {
      if (this.formValidate.agree === "" || !this.formValidate.agree) {
        ...
        updateAgreementApi("sys_coupon_agree", this.formValidate)
          .then(async (res) => {
            this.fullscreenLoading = false;
