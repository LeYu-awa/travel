...
<script>
import { assistReviewDetailApi, assistProductStatusApi } from '@/api/product'
const defaultObj = {
  image: '',
  slider_image: [],
  store_name: '',
...
export default {
  ...
  methods: {
    onSubmit() {
      this.ruleForm.id = this.projectData.product_assist_id
      assistProductStatusApi(this.ruleForm).then(res => {
    ...
    getInfo(id) {
      this.proId = id
      this.loading = true
      assistReviewDetailApi(id).then(res => {
        this.projectData = res.data
        if (this.projectData.spec_type === 0) {
          this.OneattrValue = res.data.attrValue
          this.OneattrValue[0]['price'] = this.OneattrValue[0]['assistSku']['assist_price']
