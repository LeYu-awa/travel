<template>
  <div class="box">
    ...
    <el-card class="mt14" style="flex: 1">
      <div class="mb14">
        <router-link :to="{
          path: roterPre + '/content/course/special/article/addArticle2', query: {
            type: 2
          }
        }">
          <el-button size="small" type="primary">添加音频</el-button>
        </router-link>
  ...
  methods: {
    ...
    onErtion(name, row) {
      switch (name) {
        case 'edit':
          this.$router.push({
            path: this.roterPre + '/content/course/special/article/addArticle2/' + row.id,
            query: {
              type: 2
            }
          })
    ...
    getList(num) {
      ...
      getLessonSubjectList({ type: 2, ...this.tableFrom })
        .then((res) => {
