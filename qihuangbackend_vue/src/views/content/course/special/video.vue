<template>
  <div class="box">
    ...
    <el-card class="mt14" style="flex: 1">
      <div class="mb14">
        <router-link :to="{
          path: roterPre + '/content/course/special/article/addArticle1', query: {
            type: 1
          }
        }">
  ...
  methods: {
    ...
    onErtion(name, row) {
      switch (name) {
        case 'edit':
          this.$router.push({
            path: this.roterPre + '/content/course/special/article/addArticle1/' + row.id,
            query: {
              type: 1
            }
          })
    ...
    getList(num) {
      ...
      getLessonSubjectList({ type: 1, ...this.tableFrom })
        .then((res) => {
