...
import { diseaseCategoryList,diseaseCategoryDelete, diseaseCategoryEdit } from '@/api/content'
...
export default {
  ...
  methods: {
      ...
      switch (name) {
        ...
        case 'del':
          this.$confirm('是否删除此数据?', '提示', {
              ...
              this.IfdiseaseCategoryDelete(row.id)
    ...
    // 删除接口
    IfdiseaseCategoryDelete(str) {
      diseaseCategoryDelete(str).then(res => {
        this.$message({
          type: 'success',
          message: '删除成功!'
        });
    ...
    getList(num) {
      ...
      diseaseCategoryList(this.tableFrom)
        .then((res) => {
