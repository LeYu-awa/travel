...
<script>
import { efficacyAdd, efficacyEdit } from "@/api/content";
...
export default {
  ...
  methods: {
    ...
    onSubmitForm() {
      this.fApi.submit((formData, fApi) => {
        if (formData.id) {
          ...
          this.IfaddefficacyAdd(formData);
        }
      });
    },
    cancel() {
    ...
    IfaddefficacyAdd(data) {
      efficacyAdd(data).then(res => {
        this.$emit("confirm");
      });
    },
    IfupdateEditGift(data) {
      efficacyEdit(data).then(res => {
        this.$emit("confirm");
      });
    }
  }
