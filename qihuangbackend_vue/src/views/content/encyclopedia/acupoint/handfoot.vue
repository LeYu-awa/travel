  ...
  <div class="box">
    ...
    <el-card class="mt14" style="flex: 1">
      ...
      <dTable :table-data="tableData" :table-header-data="tableHeaderData" :showLoading="showLoading">
        ...
        <template #type="{ row }">{{ type[row.type] }}</template>
...
import { handfootList, handfootDelete, handfootEdit } from '@/api/content'
...
export default {
  ...
  data() {
    return {
      ...
      tableHeaderData: [
        ...
        { prop: 'code', label: '分类编码', columnType: 'text', align: 'left', 'min-width': '249' },
        { prop: 'type', label: '类别', columnType: 'slot', align: 'left', 'min-width': '249' },
  ...
  methods: {
    ...
    // 删除接口
    IfhandfootDelete(str) {
      handfootDelete(str).then(res => {
        this.$message({
          type: 'success',
          message: '删除成功!'
        });
