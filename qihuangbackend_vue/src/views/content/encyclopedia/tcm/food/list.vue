...
import { foodCategoryList,foodCategoryDelete, foodCategoryEdit } from '@/api/content'
...
export default {
  ...
  methods: {
      ...
      switch (name) {
        ...
        case 'del':
          this.$confirm('是否删除此数据?', '提示', {
              ...
              this.IffoodCategoryDelete(row.id)
    ...
    // 删除接口
    IffoodCategoryDelete(str) {
      foodCategoryDelete(str).then(res => {
        this.$message({
          type: 'success',
          message: '删除成功!'
        });
    ...
    getList(num) {
      ...
      foodCategoryList(this.tableFrom)
        .then((res) => {
