...
<script>
import { foodCategoryAdd, foodCategoryEdit } from "@/api/content";
...
export default {
  ...
  methods: {
    ...
    onSubmitForm() {
      this.fApi.submit((formData, fApi) => {
        if (formData.id) {
          ...
          this.IfaddfoodCategoryAdd(formData);
        }
      });
    },
    cancel() {
    ...
    IfaddfoodCategoryAdd(data) {
      foodCategoryAdd(data).then(res => {
        this.$emit("confirm");
      });
    },
    IfupdateEditGift(data) {
      foodCategoryEdit(data).then(res => {
        this.$emit("confirm");
      });
    }
  }
