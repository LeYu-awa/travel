  ...
  <div class="box">
    <div class="selCard">
      <el-form :model="tableFrom" ref="searchForm" inline size="small" label-width="85px" >
        <el-form-item label="功效名称：" prop="name">
          <el-input
...
import editpopUpCategory from './editpopUpCategory.vue'
import { medicinalDietEfficacyList,medicinalDietEfficacyDelete, medicinalDietEfficacyEdit } from '@/api/content'
...
export default {
  ...
  data() {
    return {
      ...
      tableData: [],
      tableHeaderData: [
        { prop: 'name', label: '功效名称', columnType: 'text', align: 'left', 'min-width': '249' },
  ...
  methods: {
      ...
      switch (name) {
        ...
        case 'del':
          this.$confirm('是否删除此数据?', '提示', {
              ...
              this.IfmedicinalDietEfficacyDelete(row.id)
    ...
    // 删除接口
    IfmedicinalDietEfficacyDelete(str) {
      medicinalDietEfficacyDelete(str).then(res => {
        this.$message({
          type: 'success',
          message: '删除成功!'
        });
    ...
    getList(num) {
      ...
      medicinalDietEfficacyList(this.tableFrom)
        .then((res) => {
