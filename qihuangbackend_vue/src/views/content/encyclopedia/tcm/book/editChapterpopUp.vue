...
<script>
import { bookChapterAdd, bookChapterEdit } from "@/api/content";
...
export default {
  ...
  methods: {
    ...
    onSubmitForm() {
      this.fApi.submit((formData, fApi) => {
        if (formData.id) {
          ...
          this.IfaddbookChapterAdd(formData);
        }
      });
    },
    cancel() {
    ...
    IfaddbookChapterAdd(data) {
      bookChapterAdd(data).then(res => {
        this.$emit("confirm");
      });
    },
    IfupdateEditGift(data) {
      bookChapterEdit(data).then(res => {
        this.$emit("confirm");
      });
    }
  }
