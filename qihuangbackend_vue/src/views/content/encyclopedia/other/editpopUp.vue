...
<script>
import { otherArticleCategoryAdd, otherArticleCategoryEdit } from "@/api/content";
...
export default {
      ...
      FormData: [
        ...
        },
        {
          ...
          props: {
            ...
            maxLength: 1,
            title: "请选择图标：",
            src: "/admin/setting/uploadPicture?field=logo&type=1",
            width: "1000px",
            height: "600px",
            footer: false,
            icon: "el-icon-camera",
  ...
  methods: {
    ...
    onSubmitForm() {
      this.fApi.submit((formData, fApi) => {
        if (formData.id) {
          ...
          this.IfaddotherArticleCategoryAdd(formData);
        }
      });
    },
    cancel() {
    ...
    IfaddotherArticleCategoryAdd(data) {
      otherArticleCategoryAdd(data).then(res => {
        this.$emit("confirm");
      });
    },
    IfupdateEditGift(data) {
      otherArticleCategoryEdit(data).then(res => {
        this.$emit("confirm");
      });
    }
  }
