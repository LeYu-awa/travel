...
import { otherArticleList,otherArticleDelete, otherArticleEdit } from '@/api/content'
...
export default {
  ...
  data() {
    return {
      ...
      tableHeaderData: [
        ...
        { prop: 'name', label: '分类', columnType: 'text', align: 'left', 'min-width': '249' },
  ...
  methods: {
      ...
      switch (name) {
        ...
        case 'del':
          this.$confirm('是否删除此数据?', '提示', {
              ...
              this.IfotherArticleDelete(row.id)
    ...
    // 删除接口
    IfotherArticleDelete(str) {
      otherArticleDelete(str).then(res => {
        this.$message({
          type: 'success',
          message: '删除成功!'
        });
    ...
    getList(num) {
      ...
      otherArticleList(this.tableFrom)
        .then((res) => {
