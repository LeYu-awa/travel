...
import { otherArticleCategoryList,otherArticleCategoryDelete, otherArticleCategoryEdit } from '@/api/content'
...
export default {
  ...
  data() {
    return {
      ...
      tableHeaderData: [
        ...
        { prop: 'logo', label: '图标', columnType: 'image', align: 'left', 'min-width': '249' },
  ...
  methods: {
      ...
      switch (name) {
        ...
        case 'del':
          this.$confirm('是否删除此数据?', '提示', {
              ...
              this.IfotherArticleCategoryDelete(row.id)
    ...
    // 删除接口
    IfotherArticleCategoryDelete(str) {
      otherArticleCategoryDelete(str).then(res => {
        this.$message({
          type: 'success',
          message: '删除成功!'
        });
    ...
    getList(num) {
      ...
      otherArticleCategoryList(this.tableFrom)
        .then((res) => {
