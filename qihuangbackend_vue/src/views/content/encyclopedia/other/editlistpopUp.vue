...
<script>
import { otherArticleAdd, otherArticleEdit } from "@/api/content";
...
export default {
  ...
  methods: {
    ...
    async getOptions() {
      let { data } = await otherArticleCategoryList({limit: 1000});
      let arr = [];
      data.list.map(v => {
        arr.push({
          label: v.name,
    ...
    onSubmitForm() {
      this.fApi.submit((formData, fApi) => {
        if (formData.id) {
          ...
          this.IfaddotherArticleAdd(formData);
        }
      });
    },
    cancel() {
    ...
    IfaddotherArticleAdd(data) {
      otherArticleAdd(data).then(res => {
        this.$emit("confirm");
      });
    },
    IfupdateEditGift(data) {
      otherArticleEdit(data).then(res => {
        this.$emit("confirm");
      });
    }
  }
