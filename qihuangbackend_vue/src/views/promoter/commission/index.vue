...
export default {
  ...
  methods: {
    getInfo() {
      ...
      getEextensionApi('sys_extension_agree').then(res => {
        const info = res.data
        this.formValidate = {
          agree: info.sys_extension_agree,
        }
    ...
    handleSubmit(name) {
      if(this.formValidate.agree === '' || !this.formValidate.agree){
        ...
        updateEextensionApi('sys_extension_agree', this.formValidate).then(async res => {
          this.fullscreenLoading = false
