      ...
      <div v-loading="loading" v-if="psInfo">
        <div class="head">
          <div class="full">
            <img
              class="order_icon"
              :src="psInfo.avatar ? psInfo.avatar : moren"
              alt=""
            />
            <div class="text">
              ...
              <div class="acea-row">
                <img
                  v-if="psInfo.member_icon"
                  class="svip-grade"
                  :src="psInfo.member_icon"
                  alt=""
                />
            ...
            <div v-if="isUser && is_area == 0">
              ...
              <el-dropdown @command="handleCommand" class="ml10">
                ...
                <el-dropdown-menu slot="dropdown">
                  ...
                  <el-dropdown-item command="reference"
                    >修改推荐人</el-dropdown-item
                  >
                  <el-dropdown-item command="password"
                    >修改密码</el-dropdown-item
                  >
                  <el-dropdown-item command="svipLevel"
                    >编辑会员等级</el-dropdown-item
                  >
                  <el-dropdown-item command="svipSet"
                    >付费会员设置</el-dropdown-item
                  >
                </el-dropdown-menu>
              </el-dropdown>
            </div>
              ...
              <div class="section">
                ...
                <ul class="list">
                  ...
                  <li class="item">
                    ...
                    <div class="value">
                      <span
                        v-for="(item, index) in psInfo.userLabel"
                        :key="index"
                      >
                        {{ item.label_name }}
                      </span>
                  ...
                  <li
                    v-if="psInfo.is_svip == 1 || psInfo.is_svip == 2"
                    class="item"
                  >
                    <div>会员到期时间：</div>
              ...
              <div class="section">
                ...
                <el-row :gutter="24">
                  <el-form-item label="推广员：" prop="is_promoter">
                    <el-radio-group v-model="psInfo.is_promoter">
                ...
                <el-row :gutter="24">
                  <el-form-item label="用户状态：" prop="status">
                    <el-radio-group v-model="psInfo.status">
                      <el-radio :label="1" class="radio">开启</el-radio>
                      <el-radio :label="0">关闭</el-radio>
                    </el-radio-group>
                  </el-form-item>
                  ...
                  >
                    ...
                    >
                      <el-radio-group v-model="item.value">
                        <el-radio
                          :label="key"
                          v-for="(radio, key) in item.content"
                          :key="key"
                        >
                          <span>{{ radio }}</span>
                        </el-radio>
                ...
                >
                  <template slot-scope="scope">
                    <div v-if="item.key == 'number'">
                      ...
                      >
                      <span
                        v-else-if="scope.row.pm == 0"
                        style="color:rgb(130, 228, 147);"
                        >-{{ scope.row.number }}</span
                      >
                    </div>
            ...
            <template v-if="activeName == 'growth'">
              <el-table :data="tableData.data" size="small">
                ...
                >
                  <template slot-scope="scope">
                    <div v-if="item.key == 'number'">
                      <span v-if="scope.row['pm'] > 0" style="color: #ff0000;"
                        >+{{ scope.row[item.key] }}</span
                      >
                      <span v-else style="color: #00C050;"
                        >-{{ scope.row[item.key] }}</span
                      >
                    </div>
                    ...
                    <template slot-scope="scope">
                      ...
                      >
                        <div class="demo-image__preview">
                          <el-image
                            v-if="scope.row.spu"
                            :src="scope.row.spu && scope.row.spu.image"
                            :preview-src-list="[scope.row.spu.image]"
                          />
                        </div>
                        <span
                          class="priceBox"
                          style="margin-left: 10px;width: 220px;"
                          >{{ scope.row.spu && scope.row.spu.store_name }}</span
                        >
                      </div>
...
export default {
  ...
  mounted() {
    this.is_area = localStorage.getItem("is_area");
  },
  beforeUpdate() {
    this.$nextTick(() => {
  ...
  methods: {
    ...
    getInfo(key) {
      ...
      switch (key) {
        case "record":
          userOrderApi(this.uid, this.tableFrom)
            .then(res => {
          ...
          break;
        case "recommend":
          modifyUserRefLog(this.uid, this.tableFrom)
            .then(res => {
              this.tableData.data = res.data.list;
              this.tableData.total = res.data.count;
              this.columns = [
          ...
          break;
        case "signRecord":
          userSignLogApi(this.uid, this.tableFrom)
            .then(res => {
              this.tableData.data = res.data.list;
              this.tableData.total = res.data.count;
              this.columns = [
          ...
          break;
        case "coupon":
          userCouponApi(this.uid, this.tableFrom)
            .then(res => {
              this.tableData.data = res.data.list;
              this.tableData.total = res.data.count;
              this.columns = [
          ...
          break;
        case "balance":
          userBillApi(this.uid, this.tableFrom)
            .then(res => {
              this.tableData.data = res.data.list;
              this.tableData.total = res.data.count;
              this.columns = [
        ...
        case "growth":
          ...
          memberGrowthLog(this.tableFrom)
            .then(res => {
              this.tableData.data = res.data.list;
              this.tableData.total = res.data.count;
              this.columns = [
          ...
          break;
        case "detailed":
          userPointsApi(this.uid, this.tableFrom)
            .then(res => {
              this.tableData.data = res.data.list;
              this.tableData.total = res.data.count;
              this.columns = [
          ...
          break;
        case "footsteps":
          userHistoryApi(this.uid, this.tableFrom)
            .then(res => {
              this.tableData.data = res.data.list;
              this.tableData.total = res.data.count;
              this.columns = [
    ...
    onEdit() {
      ...
      that.loading = true;
      userEditApi(this.uid, this.psInfo)
        .then(res => {
          this.isEdit = false;
          this.isDetail = true;
          this.loading = false;
