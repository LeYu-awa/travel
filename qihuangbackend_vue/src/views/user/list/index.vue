<template>
  <div class="divBox">
          ...
          <div>
            <el-form-item label="用户搜索：">
              <el-input
                ...
                clearable
                @keyup.enter.native="changeSearch(1)"
              >
                <el-select
                  v-model="select"
                  slot="prepend"
        ...
        <div v-else class="acea-row search-form">
          <div class="search-form-box">
            ...
            <el-form-item label="信息补充：">
              <el-input
                placeholder="请输入内容"
                v-model="userFrom.fields_value"
                class="input-with-select selWidth"
                clearable
                @keyup.enter.native="changeSearch(1)"
              >
                <el-select
                  v-model="userFrom.fields_type"
                  slot="prepend"
                  clearable
                  placeholder="请选择"
                >
    ...
    <el-card class="mt14">
      ...
      >
          ...
          <template slot-scope="props">
            <el-form label-position="left" inline class="demo-table-expand">
              ...
              <el-form-item label="标签：">
                <span
                  v-for="(item, index) in props.row.label"
                  :key="index"
                  v-text="item"
                />
              </el-form-item>
              ...
              :value="
                !scope.row.cancel_time &&
                  (checkedIds.indexOf(scope.row.uid) > -1 ||
                    (chkName === 'duo' &&
                      noChecked.indexOf(scope.row.uid) === -1))
              "
          ...
          <template slot-scope="{ row }">
            <span>{{
              row.user_type === "routine"
                ...
                ? "公众号"
                : row.user_type === "app" || row.user_type === "App"
                ? "App"
                : row.user_type === "pc"
                ? "PC"
                : "H5"
        ...
        <el-table-column
          ...
          :sort-method="
            (a, b) => {
              return a.now_money - b.now_money;
            }
          "
        />
        <el-table-column prop="integral" label="当前可用积分" min-width="100" />
        <el-table-column prop="addres" label="地址" min-width="130" />
          ...
          <template slot-scope="scope">
            ...
            <el-dropdown v-if="is_area == 0">
              ...
              <el-dropdown-menu slot="dropdown">
                ...
                <el-dropdown-item
                  v-if="!scope.row.cancel_time"
                  @click.native="setGroup(scope.row)"
                  >设置分组</el-dropdown-item
                >
                <el-dropdown-item
                  v-if="!scope.row.cancel_time"
                  @click.native="setLabel(scope.row)"
                  >设置标签</el-dropdown-item
                >
                <el-dropdown-item
                  v-if="!scope.row.cancel_time"
                  @click.native="setModify(scope.row)"
                ...
                <el-dropdown-item
                  v-if="!scope.row.cancel_time"
                  @click.native="setMember(scope.row)"
                  >编辑会员等级</el-dropdown-item
                >
                <el-dropdown-item
                  v-if="!scope.row.cancel_time"
                  @click.native="giveMember(scope.row)"
    ...
    >
      <news-category
        v-if="visible"
        :is-show-send="isShowSend"
        :wechat-ids="wechatIds"
        :user-ids="ids"
        :max-cols="maxCols"
      />
...
export default {
  ...
  methods: {
    ...
    // 修改密码
    setPassword(row) {
      this.$modalForm(modifyUserPassword(row.uid)).then(() =>
        this.changeSearch()
      );
    },
    // 修改余额
    setMoney(row) {
      this.$modalForm(changeNowMoneyApi(row.uid)).then(() =>
        this.changeSearch()
      );
    },
    // 修改积分余额
    changeIntegral(row) {
      this.$modalForm(changeNowIntegralApi(row.uid)).then(() =>
        this.changeSearch()
      );
    },
    // 列表
