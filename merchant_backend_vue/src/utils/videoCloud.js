...
import { uploadVideoOfLocal } from '@/api/product'
...
export default {
  videoUpload(config) {
    ...
    switch (config.type) {
      case 'COS':
        ...
        break;
      case 'OSS':
        result = this.ossHttp(config.evfile, config.res.data, config.uploading);
        break;
      case 'OBS':
        result = this.obsHttp(config.evfile, config.res.data, config.uploading);
        break;
      case 'US3':
        result = this.us3Http(config.evfile, config.res.data, config.uploading);
        break;
      case 'JDOSS':
        result = this.uploadMp4ToLocal(config.evfile, config.res.data, config.uploading);
        break;
      case 'CTOSS':
        result = this.obsHttp(config.evfile, config.res.data, config.uploading);
        break;
    ...
    return new Promise((resolve, reject) => {
      cos.sliceUploadFile({
        ...
        if (err) {
          ...
          resolve({ url: config.cdn ? config.cdn + filename : 'http://' + data.Location, ETag: data.ETag })
        }
      })
    })
  },
  ...
  cosHttp(evfile, res, videoIng) {
    ...
    const fileObject = evfile.target.files[0]
    const Key = fileObject.name
    const pos = Key.lastIndexOf('.')
    let suffix = ''
    if (pos !== -1) {
    ...
    const filename = new Date().getTime() + suffix
    const data = res.data
    const XCosSecurityToken = data.credentials.sessionToken
    const url = data.url + camSafeUrlEncode(filename).replace(/%2F/g, '/')
    ...
    return new Promise((resolve, reject) => {
      xhr.onload = function() {
        if (/^2\d\d$/.test('' + xhr.status)) {
          ...
          videoIng(false, 0)
          resolve({ url: data.cdn ? data.cdn + camSafeUrlEncode(filename).replace(/%2F/g, '/') : url, ETag: ETag })
        } else {
  ...
  qiniuHttp(evfile, res, videoIng) {
    const uptoken = (res.data&&res.data.token) || res.token
    ...
    const filename = new Date().getTime() + suffix
    const fileUrl = (res.data&&res.data.domain + '/' + filename) || (res.domain + '/' + filename)
    const config = {
      useCdnDomain: true
    }
    const putExtra = {
    ...
    return new Promise((resolve, reject) => {
      observable.subscribe({
        ...
        complete: (result) => {
          // 接收成功后返回的信息
          videoIng(false, 0)
          let cdn = (res.data && res.data.cdn) || res.cdn
          resolve({ url: cdn ? cdn + '/' + filename : fileUrl })
        }
      })
    })
  },
  jdossHttp(evfile, res, videoIng){
    const fileObject = evfile.target.files[0]
    console.log(evfile)
    const Key = fileObject.name
    const pos = Key.lastIndexOf('.')
    ...
    const formData = new FormData()
    const data = res
    //formData.append('Key', fileObject.name)
    formData.append('key', filename)
    formData.append('x-amz-credential',data.credential)
    formData.append('x-amz-algorithm', 'AWS4-HMAC-SHA256')
    formData.append('x-amz-date', data.expiration)
    formData.append('policy', data.policy)
    formData.append('x-amz-signature', data.signature)
    formData.append('file', fileObject)
    return new Promise((resolve, reject) => {
      axios.defaults.withCredentials = false
      const url = `https://${data.storageName}.s3.${data.storageRegion}.jdcloud-oss.com`
  ...
  uploadMp4ToLocal(evfile, res, videoIng) {
    ...
    formData.append('file', fileObject)
    videoIng(true, 100)
    return uploadVideoOfLocal(formData)
  }
}
