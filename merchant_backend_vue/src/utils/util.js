...
export const setTagNavListInLocalstorage = list => {
  localStorage.setItem("mertagNaveList", JSON.stringify(list));
};
/**
 * @returns {Array} 其中的每个元素只包含路由原信息中的name, path, meta三项
 */
export const getTagNavListFromLocalstorage = () => {
  const list = localStorage.getItem("mertagNaveList");
 ...
 */
export const routeHasExist = (tagNavMerList, routeItem) => {
  let len = tagNavMerList.length;
  let res = false;
  doCustomTimes(len, index => {
    if (routeEqual(tagNavMerList[index], routeItem)) res = true;
  });
  return res;
};
