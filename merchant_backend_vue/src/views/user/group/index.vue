<template>
  <div class="divBox">
    <el-card class="box-card">
      <div class="mb20">
        <el-button size="small" type="primary" @click="onAdd">添加用户标签</el-button>
      </div>
      <el-table
...
export default {
  ...
  methods: {
    ...
    // 添加
    onAdd() {
      this.$modalForm(labelFormApi()).then(() => this.getList())
    },
    // 编辑
    onEdit(id) {
      this.$modalForm(labelEditApi(id)).then(() => this.getList())
    },
    // 删除
    handleDelete(id, idx) {
