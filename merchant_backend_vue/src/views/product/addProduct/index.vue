        ...
        <el-row v-if="currentTab == '2'">
          <el-col :span="24">
            ...
            >
              ...
              <span style="color: #F56464;">（平台设置，一级佣金{{extension_one_rate}}%，二级佣金{{extension_two_rate}}%）</span>
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item
          ...
          <el-col v-if="formValidate.spec_type === 1" :span="24" class="noForm">
            <el-form-item label="选择规格：">
              <div class="acea-row">
                <el-select v-model="selectRule" size="small">
                  <el-option
          ...
          <el-col :xl="24" :lg="24" :md="24" :sm="24" :xs="24">
            ...
            <!-- 多规格表格-->
            <el-form-item
              v-if="(formValidate.spec_type === 1 && formValidate.attr.length > 0 && $route.params.id) || (!$route.params.id && createProduct && formValidate.spec_type === 1) || (!$route.params.id && formValidate.is_copy)"
              class="labeltop"
              label="规格列表："
            >
              <el-table
              ...
              >
                ...
                <el-table-column align="center" label="图片" min-width="80">
                  <template slot-scope="scope">
                    ...
                    >
                      <div v-if="scope.row.image || scope.row.pic" class="pictrue tabPic">
            ...
            <el-form-item label="店铺关联推荐：">
               <div class="acea-row">
                ...
                >
                  ...
                  <i
                    class="el-icon-error btndel"
                    @click="deleteRecommend(index)"
                  />
                </div>
                <div
                  class="uploadCont"
                  v-if="goodList.length<18"
                >
        ...
        <el-row v-if="currentTab == '5'">
          <el-col :span="24">
            <el-form-item label="上架方式：">
              <el-radio-group v-model="formValidate.term_status">
                <el-radio :label="0">立即上架</el-radio>
                <el-radio :label="1">设置上架时间</el-radio>
                <el-radio :label="2">设置上架周期</el-radio>
              </el-radio-group>
            </el-form-item>
          </el-col>
          <el-col v-if="formValidate.term_status === 1" :span="24">
            <el-form-item label="上架时间段：">
              <el-date-picker
          ...
          <el-col v-if="formValidate.term_status === 2" :span="24">
            <el-form-item label="每日上架时段：">
              <el-time-picker
                v-model="formValidate.start_hours"
                placeholder="开始时间"
                format="HH:mm"
                value-format="HH:mm"
                size="small"
              ...
              <span style="margin: 0 10px;">至</span>
              <el-time-picker
                v-model="formValidate.end_hours"
                placeholder="结束时间"
                format="HH:mm"
                value-format="HH:mm"
                size="small"
              />
              <div style="color:#999;font-size:12px;margin-top:10px;">设置后商品将在每日此时段内自动上架，支持跨天（如22:00至次日06:00）</div>
            </el-form-item>
          </el-col>
          <el-col>
  ...
  methods: {
    ...
    generate() {
      ...
      generateAttrApi(id,{ attrs: this.formValidate.attr,product_type: this.formValidate.product_type })
        .then((res) => {
          let info = res.data
          this.generateArr = this.formValidate.is_copy ? this.formValidate.value : res.data.value
          ...
          if (!this.$route.params.id && this.formValidate.spec_type === 1) { 
            this.ManyAttrValue.map((item) => {
              item.image = (item.image||item.pic) || this.formValidate.slider_image[0];
              // this.$set(item, 'select', true)
            });
            this.oneFormBatch[0].image = this.oneFormBatch[0].image || this.formValidate.slider_image[0];
          }
					this.getAttr();
        })
        .catch((res) => {
    ...
    getInfo() {
      ...
      productDetailApi(this.$route.params.id)
        .then(async res => {
          ...
          if(info.mer_form_id){
            let time = new Date().getTime() * 1000
            let formUrl = `${this.baseURL}/pages/admin/system_form/index?inner_frame=1&time=${time}&form_id=${info.mer_form_id}`;
            this.formUrl = formUrl;
            // associatedFormInfo(info.mer_form_id).then((res) => {
            //   this.formData = res.data
            // })
            // .catch((res) => {
            //   this.$message.error(res.message)
    ...
    handleSubmit(name) {
      ...
      // 上架方式数据清理
      if (this.formValidate.term_status === 0) {
        this.formValidate.start_time = '';
        this.formValidate.end_time = '';
        this.formValidate.start_hours = '';
        this.formValidate.end_hours = '';
      } else if (this.formValidate.term_status === 1) {
        this.formValidate.start_hours = '';
        this.formValidate.end_hours = '';
      } else if (this.formValidate.term_status === 2) {
        this.formValidate.start_time = '';
        this.formValidate.end_time = '';
      }
      this.$refs[name].validate(valid => {
