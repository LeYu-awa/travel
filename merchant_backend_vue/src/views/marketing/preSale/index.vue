  ...
  <div class="divBox">
    <div class="selCard">
      <el-form :model="tableFrom" ref="searchForm" size="small" label-width="85px" inline>
        ...
        <el-form-item label="关键字：" prop="keyword">
          <el-input v-model="tableFrom.keyword" placeholder="请输入预售商品名称/ID" clearable class="selWidth" @keyup.enter.native="getList(1)" />
        </el-form-item>
        <el-form-item>
    ...
    <el-card class="mt14">
      ...
      <router-link :to=" { path:`${roterPre}` + '/marketing/presell/create' } ">
        <el-button size="small" type="primary" class="mt5">
          <i class="add">+</i> 添加预售商品
        </el-button>
      </router-link>
      <el-table v-loading="listLoading" :data="tableData.data" class="mt20" size="small" :row-class-name="tableRowClassName" @rowclick.stop="closeEdit">
        ...
        <el-table-column prop="store_name" label="商品名称" min-width="250" />
        ...
        <el-table-column label="预售活动日期" min-width="120">
          <template slot-scope="scope">
        ...
        <el-table-column label="操作" min-width="240" fixed="right">
          <template slot-scope="scope">
            ...
            <el-button type="text" size="small" @click="goDetail(scope.row.product_presell_id)">详情</el-button>
            <el-button v-if="scope.row.product.presell_status !== 1" type="text" size="small" @click="handleDelete(scope.row.product_presell_id,scope.$index)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <div class="block">
    ...
    <el-dialog v-if="dialogVisible" title="预售商品详情" center :visible.sync="dialogVisible" width="660px">
    ...
    >
      <el-form ref="labelForm" :model="labelForm" size="small" @submit.native.prevent>
        <el-form-item>
          <el-select v-model="labelForm.mer_labels" clearable multiple placeholder="请选择" style="width: 100%;">
            <el-option
              v-for="item in labelList"
              :key="item.id"
              :label="item.name"
