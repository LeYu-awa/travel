<template>
  <el-dialog title="查看详情" :visible.sync="dialogVisible" width="660px" v-if="dialogVisible">
    <el-table v-loading="listLoading" :data="tableData.data" size="small">
      <el-table-column prop="uid" label="ID" min-width="80" />
      <el-table-column prop="nickname" label="用户名称" min-width="120">
          <template slot-scope="scope">
            <span v-if="scope.row.is_initiator == 1" class="initiator">团长</span><span>{{ scope.row.nickname }}</span>
          </template>
        </el-table-column>
      ...
      <el-table-column label="订单状态" min-width="90">
          <template slot-scope="scope">
            <span v-if="scope.row.orderInfo && scope.row.orderInfo.status == -1" class="refuned"> 已退款</span>
