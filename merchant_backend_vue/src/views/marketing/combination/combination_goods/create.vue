    ...
    <el-card class="box-card">
      ...
      <el-form ref="formValidate" v-loading="fullscreenLoading" class="formValidate mt20" :rules="ruleValidate" :model="formValidate" label-width="160px" @submit.native.prevent>
        ...
        <div v-show="currentTab === 1">
          ...
          <el-row :gutter="24">
            <el-col class="sp100">
              <el-form-item label="拼团简介：" prop="store_info">
                <el-input v-model="formValidate.store_info" size="small" type="textarea" :rows="3" placeholder="请输入秒杀活动简介" />
              </el-form-item>
            </el-col>
          </el-row>
          <el-row :gutter="24">
            ...
            <el-col v-if="(formValidate.delivery_free == 0 && ((formValidate.delivery_way.length == 2) || (formValidate.delivery_way.length == 1 && formValidate.delivery_way[0] == 2))) && formValidate.type == 0" :span="24">
              <el-form-item label="运费模板：" prop="temp_id">
                <div class="acea-row">
                  <el-select v-model="formValidate.temp_id" size="small" placeholder="请选择" class="selWidthd mr20">
            ...
            <el-col :span="24">
              <el-form-item label="平台保障服务：">
                <div class="acea-row">
                  <el-select v-model="formValidate.guarantee_template_id" size="small" placeholder="请选择" clearable class="selWidthd mr20">
                    <el-option
            ...
            <el-col :span="24">
              <el-form-item label="拼团时效(单位：小时)：" prop="time">
                <el-input-number v-model="formValidate.time" size="small" class="mr10" :min="1" placeholder="请输入时效" />
            ...
            <el-col :span="24">
              <el-form-item label="拼团人数：" prop="buying_count_num">
                <el-input-number v-model="formValidate.buying_count_num" size="small" class="mr10" :min="2" placeholder="请输入人数" @change="calFictiCount" />
                <span class="item_desc">单次拼团需要参与的用户数</span>
              </el-form-item>
            </el-col>
            <el-col :span="24">
              <el-form-item label="活动期间限购件数：" prop="pay_count">
                <el-input-number v-model="formValidate.pay_count" size="small" class="mr10" :min="1" placeholder="请输入数量" />
                <span class="item_desc">该商品活动期间内，用户可购买的最大数量。例如设置为4，表示本地活动有效期内，每个用户最多可购买4件</span>
              </el-form-item>
            </el-col>
            <el-col :span="24">
              <el-form-item label="单次限购件数：" prop="once_pay_count">
                <el-input-number v-model="formValidate.once_pay_count" size="small" class="mr10" :min="1" :max="formValidate.pay_count" placeholder="请输入数量" />
          ...
          <el-row v-if="combinationData.ficti_status == 1" :gutter="24">
            <el-col :span="24">
              <el-form-item label="虚拟成团：">
                <el-switch v-model="formValidate.ficti_status" :width="55" active-text="开启" inactive-text="关闭" />
              </el-form-item>
            </el-col>
          </el-row>
          <el-row v-if="combinationData.ficti_status == 1" :gutter="24">
            <el-col v-if="formValidate.ficti_status == 1" :span="24">
              <el-form-item label="虚拟成团补齐人数：" prop="ficti_num">
                <el-input-number v-model="formValidate.ficti_num" size="small" class="mr10" :min="0" :max="max_ficti_num" placeholder="请输入数量" />
                <span class="item_desc">拼团时效到时，系统自动补齐的最大拼团人数,平台规定虚拟成团最大比例为{{100-combinationData.group_buying_rate}}%</span>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row :gutter="24">
            <el-col :span="24">
              <el-form-item label="排序：" prop="sort">
                <el-input-number v-model="formValidate.sort" size="small" :min="0" placeholder="请输入排序数值" />
              </el-form-item>
            </el-col>
            <el-col :span="24">
          ...
          <!--系统表单-->
          <el-row :gutter="24" v-if="formData.length>0 && currentTab===1 && formValidate.mer_form_id">
            <el-col>
              <el-form-item label="关联系统表单：">
                ...
                <span class="explanation">如需改活动商品系统表单，请前往对应普通商品>编辑>修改系统表单即可实现</span>
              </el-form-item>
            </el-col>
            <el-col :span="16">
              <el-form-item>
              ...
              >
                ...
                <el-table-column align="center" label="市场价" min-width="80">
                  ...
                  <el-table-column align="center" label="拼团价" min-width="80">
                    <template slot-scope="scope">
                      <el-input v-model="scope.row['active_price']" size="small" type="number" class="priceBox" :min="0" :max="scope.row['price']" />
                    </template>
                  </el-table-column>
                  ...
                  <el-table-column align="center" label="限量" min-width="80">
                    <template slot-scope="scope">
                      <el-input v-model.number="scope.row['stock']" size="small" type="number" class="priceBox" :max="scope.row['old_stock']" :min="0" />
                    </template>
                  </el-table-column>
            ...
            <el-col :xl="24" :lg="24" :md="24" :sm="24" :xs="24">
              ...
              <el-form-item v-if="formValidate.spec_type === 0" label="规格列表：">
                <el-table :data="OneattrValue" border class="tabNumWidth" size="mini">
                  ...
                  <el-table-column align="center" label="拼团价" min-width="80">
                    <template slot-scope="scope">
                      <el-input v-model="scope.row['active_price']" size="small" type="number" class="priceBox" :min="0" :max="scope.row['price']" @blur="limitPrice(scope.row)" />
                    </template>
                  </el-table-column>
                  ...
                  <el-table-column align="center" label="限量" min-width="80">
                    <template slot-scope="scope">
                      <el-input v-model.number="scope.row['stock']" size="small" type="number" class="priceBox" :max="scope.row['old_stock']" :min="0" @change="limitInventory(scope.row)" />
                    </template>
                  </el-table-column>
                  <el-table-column align="center" label="商品编号" min-width="80">
                    <template slot-scope="scope">
          ...
          <el-row :gutter="24">
            <el-form-item v-if="formValidate.spec_type === 1" label="规格列表：">
              <el-table ref="multipleSelection" :data="ManyAttrValue" size="small" tooltip-effect="dark" :row-key="(row) => { return row.id }" @selection-change="handleSelectionChange">
                ...
                <el-table-column align="center" label="限量" min-width="80">
                  <template slot-scope="scope">
                    <el-input v-model.number="scope.row['stock']" size="small" type="number" class="priceBox" :min="0" :max="scope.row['old_stock']" @change="limitInventory(scope.row)"/>
                  </template>
                </el-table-column>
                <el-table-column align="center" label="商品编号" min-width="80">
                  <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    // 关联的表单信息
    getFormInfo(){
      associatedFormInfo(this.formValidate.mer_form_id).then((res) => {
        this.formData = res.data
        let time = new Date().getTime() * 1000
        let formUrl = `${this.baseURL}/pages/admin/system_form/index?inner_frame=1&time=${time}&form_id=${this.formValidate.mer_form_id}`;
