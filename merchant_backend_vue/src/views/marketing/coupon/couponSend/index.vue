<template>
  <div class="divBox">
    <div class="selCard">
      <el-form :model="tableFrom" ref="searchForm" size="small" inline label-width="90px">
        <el-form-item label="时间选择：">
          <el-date-picker v-model="timeVal" value-format="yyyy/MM/dd" format="yyyy/MM/dd" size="small" type="daterange" clearable placement="bottom-end" placeholder="自定义时间" style="width: 280px;" :picker-options="pickerOptions" @change="onchangeTime" />     
        </el-form-item>
        <el-form-item label="优惠券类型：" prop="coupon_type">
          <el-select v-model="tableFrom.coupon_type" placeholder="请选择" class="selWidth" clearable @change="getList(1)">
        ...
        <el-form-item label="优惠券名称：" prop="coupon_name">
          <el-input v-model="tableFrom.coupon_name" @keyup.enter.native="getList(1)" placeholder="请输入优惠券名称" class="selWidth" @change="getList(1)" clearable />             
        </el-form-item> 
        <el-form-item>
      ...
      >
        ...
        <el-table-column label="操作" min-width="150" fixed="right">
          <template slot-scope="scope">
            <el-button type="text" size="small" class="mr10" @click="details(scope.row)">详情</el-button>
            <el-button type="text" size="small" class="mr10" @click="usedRecord(scope.row.coupon_send_id)">使用记录</el-button>
          </template>
        </el-table-column>
      </el-table>
      <div class="block">
    ...
    <!--优惠券详情-->
    <el-dialog title="优惠券详情" :visible.sync="detailDialog" width="600px" v-if="detailDialog">
      <div>
        <div class="box-container">
          ...
          <div class="list sp">
            ...
            <span class="info">{{ couponDetail.coupon_price }}元</span>
          </div>
          <div class="list sp">
          ...
          <div class="list sp">
            ...
            <span class="info">{{ couponDetail.type | couponUseTypeFilter }}</span>
          </div>
          <div class="list sp">
        ...
        <div class="filter-container">
          <el-form size="small" inline label-width="120px">
            <el-form-item label="优惠券获取方式：">
                <el-select v-model="tableFromIssue.type" placeholder="请选择" size="small" class="selWidth" clearable @change="getIssueList()">
                <el-option label="全部" value="" />
      ...
      >
        ...
        >
          <template slot-scope="scope">
            ...
            <span>{{ scope.row.type == 'receive' ? '自己领取' : scope.row.type == 'send' ? '后台发送' : scope.row.type == 'new' ? '新人' : '买赠送' }}</span>
          </template>
        </el-table-column>
        <el-table-column
          label="使用情况"
...
// +----------------------------------------------------------------------
import { couponSendLstApi, issueApi } from '@/api/marketing'
...
export default {
  ...
  methods: {   
    ...
    getList(num) {
      ...
      couponSendLstApi(this.tableFrom).then(res => {
