                    ...
                    <div class="info">
                        <div class="info-item" v-for="(infos,key) in item.info" :key="key">
                            ...
                            <div class="input-box" @click="getLink(index,key,item.info)">
                                <el-input v-model="infos.value" :readonly="key==item.info.length-1?true:false" :placeholder="infos.tips" :maxlength="infos.max" v-if="configData.isCube" @blur="onBlur">
                                     <el-button v-if="key==item.info.length-1" slot="append" icon="el-icon-arrow-right"></el-button>
                                </el-input>
                                <el-input v-model="infos.value" :readonly="key==item.info.length-1?true:false" :placeholder="infos.tips" :maxlength="infos.max" v-else>
                                    <el-button v-if="key==item.info.length-1" slot="append" icon="el-icon-arrow-right"></el-button>
                                </el-input>
                            </div>
                        </div>
