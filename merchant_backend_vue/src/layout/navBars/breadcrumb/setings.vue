    ...
    >
      <el-scrollbar class="layout-breadcrumb-seting-bar el-main">
        ...
        <div
          ...
          v-if="
            getThemeConfig.layout === 'columns' ||
            getThemeConfig.layout === 'defaults'
          "
        >
...
export default {
  ...
  created() {
    ...
    this.bus.$on("layoutMobileResize", (res) => {
      if (this.$store.state.themeConfig.themeConfig.layout === res.layout)
        return false;
    ...
    console.log(
      this.$store.state.themeConfig.themeConfig.themeStyle,
      "this.$store.state.themeConfig.themeConfig.themeStyle"
    );
  ...
  methods: {
    ...
    onColorPickerChange() {
      ...
      for (let i = 1; i <= 9; i++) {
        document.documentElement.style.setProperty(
          ...
          `${useChangeColor().getLightColor(
            this.getThemeConfig.primary,
            i / 10
          )}`
        );
      }
    ...
    setLocalTheme(val) {
      ...
      console.log(this.getThemeConfig.layout, val);
    ...
    onMenuBgColorChange() {
      ...
      document.documentElement.style.setProperty(
        "--prev-bg-menuBar",
        this.getThemeConfig.menuBgColor
      );
      this.setLocalThemeConfig();
    },
    // 深色模式
    ...
    initLayoutConfig() {
      window.addEventListener("load", () => {
        ...
        if (this.$store.state.themeConfig.themeConfig.isGrayscale)
          this.onAddFilterChange("grayscale");
        // 色弱模式
        if (this.$store.state.themeConfig.themeConfig.isInvert)
          this.onAddFilterChange("invert");
        // 深色模式
        if (this.$store.state.themeConfig.themeConfig.isIsDark)
          this.onAddDarkChange();
      });
    },
    // 存储布局配置
    setLocalThemeConfig() {
    ...
    onAddFilterChange(attr) {
      ...
      const cssAttr =
        attr === "grayscale"
          ? `grayscale(${
              this.$store.state.themeConfig.themeConfig.isGrayscale ? 1 : 0
            })`
          : `invert(${
              this.$store.state.themeConfig.themeConfig.isInvert ? "80%" : "0%"
            })`;
    ...
    // 布局切换
    onSetLayout(layout) {
      Local.set("oldMerLayout", layout);
      if (this.$store.state.themeConfig.themeConfig.layout === layout)
        return false;
      ...
      this.$store.state.themeConfig.themeConfig.isDrawer = false;
      this.$store.state.themeConfig.themeConfig.columnsAsideStyle =
        "columns-card";
      this.setLocalTheme(this.$store.state.themeConfig.themeConfig.themeStyle);
    },
    // 菜单 / 顶栏背景等
...
.layout-breadcrumb-seting-bar ::v-deep .el-scrollbar__view {
  overflow-x: hidden;
}
