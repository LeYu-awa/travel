<template>
  <div ref="tagsView" class="layout-navbars-tagsview" v-if="tagsViewList.length>0">
    <i
      v-if="scrollTagIcon"
      class="direction el-icon-arrow-left"
      @click="scrollTag('left')"
    ></i>
    <el-scrollbar
      ref="scrollbarRef"
    ...
    <i
      v-if="scrollTagIcon"
      class="direction el-icon-arrow-right"
      @click="scrollTag('right')"
    ></i>
    <el-dropdown @command="clickDropdown" v-if="tagsViewList.length">
      <span class="setting-tag el-dropdown-link"
...
export default {
  ...
  mounted() {
    if (!this.$store.state.menu.tagNavMerList.length) {
      this.getTagsViewRoutes();
    }
    if (
      this.$refs.tagsView.offsetWidth &&
      this.$refs.tagsView.offsetWidth <
      this.$refs.scrollbarRef.$refs.wrap.scrollWidth
    ) {
  ...
  methods: {
    ...
    scrollTag(production) {
      ...
      if (production === "left") {
        this.$refs.scrollbarRef.$refs.wrap.scrollLeft =
          scrollLeft - 300 <= 0 ? 0 : scrollLeft - 300;
      } else {
        this.$refs.scrollbarRef.$refs.wrap.scrollLeft =
          scrollLeft + 300 >= scrollRefs ? scrollRefs : scrollLeft + 300;
      }
    },
    // tagsView 横向滚动
    ...
    addBrowserSetSession(tagNavMerList) {
      this.setTagNavList(tagNavMerList);
    },
    // 初始化设置了 tagsView 数据
    initTagsViewList() {
    ...
    addTagsView(path, to) {
      ...
      console.log(this.tagsViewList, "tagsViewList");
    },
    // 右键菜单点击时显示菜单列表
    onContextmenu(v, e) {
          ...
          if (v.path === path) {
            ...
            setTimeout(() => {
              ...
              // 否则，跳转到下一个
                ...
                this.$router.push({ path: arr[k].path, query: arr[k].query });
            }, 0);
          }
        }
      });
      this.setTagNavList(this.tagsViewList);
      //   this.addBrowserSetSession(this.tagNavMerList);
    },
    ...
    closeAllTagsView(path) {
      ...
      this.tagsViewRoutesList.map(v => {
        if (v.meta.isAffix) {
          tagsViewList.push({ ...v });
          if (tagsViewList.some(v => v.path === path))
