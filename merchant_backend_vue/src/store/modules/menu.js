...
const closePage = (state, route) => {
  const nextRoute = getNextRoute(state.tagNavMerList, route);
  state.tagNavMerList = state.tagNavMerList.filter(item => {
...
export default {
  ...
  state: {
    ...
    //----------------------------------------------------------------
    breadCrumbList: [],
    tagNavMerList: getTagNavListFromLocalstorage() || [],
    homeRoute: {},
  ...
  mutations: {
    ...
    setTagNavList(state, list) {
      ...
      state.tagNavMerList = tagList;
      setTagNavListInLocalstorage([...tagList]);
    },
    closeTag(state, route) {
      let tag = state.tagNavMerList.filter(item => routeEqual(item, route));
    ...
    addTag(state, { route, type = "unshift" }) {
      ...
      let i = state.tagNavMerList.findIndex(item => item.path === route.path);
      if (!routeHasExist(state.tagNavMerList, router)) {
        if (type === "push")
          if (i < 1) state.tagNavMerList.push(router);
          else {
            return;
            // if (router.name === homeName) state.tagNavMerList.unshift(router);
            // else state.tagNavMerList.splice(1, 0, router);
          }
        setTagNavListInLocalstorage([...state.tagNavMerList]);
      }
    },
    setLocal(state, lang) {
