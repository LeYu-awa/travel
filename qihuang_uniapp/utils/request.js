...
function baseRequest(url, method, data, {
  ...
  if (!noAuth) {
    //登录过期自动登录
    if (!store.state.app.token && !checkLogin() && !uni.getStorageSync('token')) {
		console.log(!store.state.app.token , !checkLogin())
      toLogin();
  ...
  if(!store.state.app.token && uni.getStorageSync('token')){
	  store.state.app.token = uni.getStorageSync('token')
  }
      ...
      success: (res) => {
        ...
        if (noVerify)
          ...
          reslove(res.data, res);
			...
			console.log(res.data.status,2222)
          toLogin();
          reject(res.data);
          ...
          reject(!store.state.app.token && !checkLogin() ? '暂未登录，请先登录' : res.data.message || '系统错误');}
      },
      fail: (message) => {
        reject('请求失败');
      }
