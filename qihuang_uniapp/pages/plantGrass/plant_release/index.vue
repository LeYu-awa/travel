			...
			<view class="release_content">
				<view>
					<view class="release_item">
							...
							<view class="input_photo acea-row row-middle">
								<block v-if="tabLength == 2 || tabLength == 3">
									<view class="pictrue" v-if="formData.video_link">
										<image class="video-bg" mode="widthFix" src="../static/images/video_bg.png">
										</image>
										...
										<view class="close_btn" @click="deleteLink"><text
												class="iconfont icon-guanbi4"></text></view>
									</view>
									<view v-else-if="!formData.video_link&&formData.image.length<=1"
										class="pictrue acea-row row-center-wrapper row-column add" @click="uploadVideo">
								...
								<!--图文或者图文加视频-->
								<block
									v-if="(tabLength == 1 && !formData.video_link) || (tabLength == 3 && !formData.video_link)">
									<view v-if="formData.image.length < 9"
										class="pictrue acea-row row-center-wrapper row-column add" @click.stop="clk(9)">
						...
						<view class="textarea">
							<textarea placeholder='分享使用体验和心得，获得更多点赞和关注哦~(1000字以内)' name="comment"
								placeholder-class='placeholder' v-model="formData.content" maxlength="1000"></textarea>
						</view>
					</view>
					<view class="release_item">
						<view class='item acea-row row-between-wrapper'>
							<view class='name'><text
									class="iconfont icon-baobeilianjie"></text>添加宝贝({{productList.length}})</view>
							<view class="select">
	...
	export default {
		...
		data() {
			return {
				...
				chunkSize: 1024 * 1024 * 1, // 分片大小 500kb
			};
		},
		computed: {
			...
			if (!this.isLogin) {
				...
				if (this.id) {
					...
					if (this.tabLength == 2 || this.tabLength == 3) {
						...
						if (this.tabLength == 2) {
							this.formData.image = getStorage('imageArr') && getStorage('imageArr').length > 0 ? [
								getStorage('imageArr')[0]
							] : []
						} else {
		...
		methods: {
			...
			uploadVideo() {
				...
				uni.chooseVideo({
					...
					success: async res => {
						...
						try {
							// 关键修复：将视频复制到应用私有目录（兼容Android 10+）
							const privatePath = await that.copyFileToPrivateDir(res.tempFilePath);
							res.tempFilePath = privatePath; // 更新为私有路径
							console.log(666, res.tempFilePath)
							if (res.size > this.chunkSize) {

								let taskId = that.generateTaskId();
								let name = this.getFileName(res.tempFilePath);
								let aa = {
								...
								const totalChunks = Math.ceil(res.size / this.chunkSize);
								// 优化：使用Promise.allSettled替代循环await，提高效率
								for(let j = 0; j < totalChunks; j++){
									await this.uploadSingleChunk(j, totalChunks)
								}
			...
			// 新增方法：将文件复制到应用私有目录
			copyFileToPrivateDir(tempFilePath) {
				return new Promise((resolve, reject) => {
					// 获取应用私有目录（Android 10+ 推荐路径）
					const targetDir = plus.io.convertLocalFileSystemURL('_doc/videos/');
					// 创建目录（如果不存在）
					plus.io.resolveLocalFileSystemURL(targetDir, dirEntry => {
						...
						plus.io.resolveLocalFileSystemURL(tempFilePath, fileEntry => {
							fileEntry.copyTo(dirEntry, fileName, newFileEntry => {
								resolve(newFileEntry.toLocalURL()); // 返回私有路径
							}, err => {
								reject(new Error(`文件复制失败: ${err.message}`));
							});
						}, err => {
						...
						// 目录不存在则创建
						plus.io.resolveLocalFileSystemURL('_doc', rootDir => {
							rootDir.getDirectory('videos', {
								create: true
							}, newDir => {
								resolve(that.copyFileToPrivateDir(tempFilePath));
							}, dirErr => {
								reject(new Error(`创建目录失败: ${dirErr.message}`));
							});
						});
					});
				});
			...
			// 修改分片上传方法（确保使用私有路径）
			uploadSingleChunk(chunkindex, chunkcount) {
				let that = this;
				return new Promise((resolve, reject) => {
					// 确保使用的是应用私有路径
					plus.io.resolveLocalFileSystemURL(this.uploadData.tempFilePath, (entry) => {
						entry.file((file) => {
							let fileReader = new plus.io.FileReader();
							const start = chunkindex * that.chunkSize;
							const end = Math.min(start + that.chunkSize, that.uploadData.fileSize);
							const blob = file.slice(start, end);

							fileReader.onload = async (e) => {
								uni.showLoading({
									title: '上传中(' + chunkindex + '/' + (chunkcount - 1) +')'
								})
								let formData = {
									base_file: e.target.result,
									chunkindex,
									chunkcount,
									chunkId: that.uploadData.chunkId,
									fileName: chunkindex + that.uploadData.fileName,
									key: that.uploadData.key,
									filesize: end - start,
								...
								let request = () => {
									uni.request({
										...
										method: 'POST',
										header: {
											[TOKENNAME]: 'Bearer ' + store
												.state.app
												.token
										},
										data: formData,
										success: (uploadRes) => {
											console.log('分片返回', chunkindex,
												uploadRes)
											if (uploadRes.statusCode ===
												200 &&
												uploadRes.data.status ===
												200) {
												let data = uploadRes.data
													.data;
												that.uploadData.etag = that
													.uploadData.etag || [];
												that.uploadData.etag.push(
													data);
												resolve();
											} else {
												console.log('错误1',
												...
												request()
												// reject(new Error(
												// 	`分片 ${chunkindex + 1} 上传失败: ${uploadRes.data.msg || '未知错误'}`
												// ));
											}
										},
										fail: (err) => {

											console.log('错误2', chunkindex,
												err)
											request()
											// reject(new Error(
											// 	`分片 ${chunkindex + 1} 上传出错: ${err.errMsg}`
											// ));
										}
									});
							...
							fileReader.onerror = (e) => {
								...
								reject(new Error(`文件读取错误: ${e.message}`));
							};

							fileReader.readAsDataURL(blob);
						}, fileErr => {
							reject(new Error(`获取文件信息失败: ${fileErr.message}`));
						});
					}, resolveErr => {
						reject(new Error(`解析文件路径失败: ${resolveErr.message}，请检查路径是否为应用私有路径`));
					});
				});
			},
			...
			uploadFilePromise() {
				let that = this;
				let formData = {};
				// 关键校验：确保filePath存在且为字符串
				if (!that.uploadData?.tempFilePath || typeof that.uploadData.tempFilePath !== 'string') {
					return Promise.reject(new Error('无效的文件路径：filePath不存在或不是字符串'));
				}

				if (this.uploadData && this.uploadData.fileSize > that.chunkSize) {
					formData = {
						chunkId: this.uploadData.chunkId,
						fileName: this.uploadData.fileName,
						chunkCount: Math.ceil(this.uploadData.fileSize / that.chunkSize),
						action: 'merge',
						key: this.uploadData.key,
						aliosstoken: this.uploadData.key,
						// file: that.uploadData?.tempFilePath,
						etags: (this.uploadData.etag || []).toString(), // 调整为正确的etag提取方式
						uploadId: this.uploadData.uploadId
					};

					console.log('formData', formData)
					return new Promise((resolve, reject) => {
						console.log('???', that.uploadData?.tempFilePath)
						uni.request({
							url: that.uploadUrl,
							method: 'POST',
							data: formData,
							...
							fail: (err) => {
								console.log(err)
								reject(new Error(`合并请求失败: ${err.errMsg}`));
							}
						});
					});
				}
				...
				return new Promise((resolve, reject) => {
					...
					uni.uploadFile({
						url: that.uploadUrl,
						filePath: that.uploadData?.tempFilePath, // 使用私有路径
						name: 'file',
						formData,
						header: {
							'content-type': 'multipart/form-data',
				...
				if (!that.isLogin) {
					...
					that.id ? updatePlantApi(that.id, value).then(res => {
						...
						if (that.tabActive == 2) {
							...
							// uni.navigateTo({
							// 	//#ifdef APP
							// 	url: '/pages/short_video/appSwiper/index?id=' + res.data.community_id +
							// 		'&user=1&uid=' + that.uid
							// 	//#endif
							// 	//#ifndef APP
