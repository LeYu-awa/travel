	...
	<view :style="viewColor">
			...
			<view class="storeinfo-wrapper">
				<view class="store-item" v-for="(item,index) in cartInfo" :key="index">
					...
					<view class="wrapper">
						...
						<view class='item acea-row row-between-wrapper'
							v-if='shippingType==0 && item.isTake == 0 && order_model == 0'>
							<view>快递费用 <text
		...
		<block v-if="coupon.status">
			<couponListWindow :coupon='coupon' :couponTitle="plantCoupon ? '平台优惠券' : '优惠券'"
				@ChangCouponsClose="ChangCouponsClose" @getCoupon="getCoupon" :openType='openType'
				:coupon_amount='coupon_amount' :coupon_number='coupon_number'></couponListWindow>
		</block>
		...
		methods: {
			...
			getConfirm: function(address_id) {
				...
				getOrderConfirm({
					...
					that.$set(that, "store_coupon_number", res.data.order[that.couponIndex].order.useCouponIds
						.length);
					that.$set(that, "coupon_number", that.plantCoupon ? that.subCoupon['0'].length : that
						.store_coupon_number);
					...
					that.$set(that, "coupon_amount", that.plantCoupon ? that.plant_coupon_amount : that
						.store_coupon_amount);
					...
					if (res.data.mer_form_id && !that.order_form.length && res.data.mer_form_info && res.data
						.mer_form_info.value) {
						...
						that.$set(that, 'order_form', (that.order_form && that.order_form.length > 0) ? that
							.order_form : formData);
					}
			...
			getaddressInfo: function() {
				...
				if (that.addressId) {
					...
					getAddressList().then(res => {
						...
						that.post = res.data.list.length > 0 ? {
							real_name: res.data.list[0].real_name,
			...
			onAddress: function() {
				...
				if (that.addressInfo.real_name) {
					...
					that.pagesUrl = '/pages/users/user_address/index?cartId=' + this.cartId + '&couponId=' + this
						.couponId;
				} else {
					uni.navigateTo({
