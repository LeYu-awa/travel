			...
			<view class="promoter-header">
				<view class="search">
					<view class="search-content acea-row row-middle">
						...
						<input type='text' v-model="keyword" placeholder='搜索用户名称' @confirm="searchSubmitValue"
							confirm-type='search' name="search" placeholder-class='placeholder' />
					</view>
				</view>
			</view>
			<view class='nav acea-row row-around'>
				<view :class="grade == 0 ? 'item on' : 'item'" @click='setType(0)'>一级
				<!-- ({{userInfo.one_level_count}}) -->
				</view>
				<view :class="grade == 1 ? 'item on' : 'item'" @click='setType(1)'>二级
				<!-- ({{userInfo.two_level_count}}) -->
				</view>
			</view>
			...
			<view class='list' v-if="recordList.length">
				...
				<block v-for="(item,index) in recordList" :key="index">
					<view class='item'>
						<view class="picTxt">
							<view class='pictrue'>
								<image :src='item.order_user_avatar ? item.order_user_avatar : "/static/images/f.png"'></image>
							</view>
							<view class='text'>
								<view class='name line1'>{{item.order_user_nickname || ''}}</view>
								<!-- <view>当前状态：<text class="num">{{item.status_text}}</text></view> -->
								<view>订单金额：<text class="num">{{item.order_amount}}</text>元</view>

								<view>创建时间：{{item.create_time}}</view>
								<!-- <view>分红金额：<text class="num">{{item.commission_amount}}</text>元</view> -->
						...
						<view class="right">
							...
							<view><text class="num"
									style="color: red;font-size: 32rpx;">+{{item.commission_amount}}</text></view>
							<view><text class="num">{{item.status_text}}</text></view>
							<!-- <view>订单金额：<text class="num">{{item.order_amount}}</text>元</view>
							<view>分红金额：<text class="num">{{item.commission_amount}}</text>元</view> -->
						</view>
					</view>
