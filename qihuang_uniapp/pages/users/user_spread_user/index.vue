	...
	<view :style="viewColor">
		<view class='my-promotion'>
			<view class="header">
				...
				<view class='profit acea-row row-between-wrapper'>
					...
					<view class='item'>
						<navigator url='/pages/users/user_spread_money/index?type=1' hover-class="none">累计已提<text
								class='iconfont icon-xiangyou font20'></text></navigator>
			...
			<view class='list acea-row row-between-wrapper'>
				...
				<navigator url='/pages/users/promoter-list/spidList' hover-class="none"
					class='item acea-row row-center-wrapper row-column'>
				...
				<navigator url='/pages/users/promoter-order/index' hover-class="none"
					class='item acea-row row-center-wrapper row-column'>
					<text class='iconfont new-iconfont icon-dingdan2'></text>
					<view>推广人订单</view>
				</navigator>
				<view @click="showPopup" class="item acea-row row-center-wrapper row-column">
					<text class='iconfont new-iconfont icon-zhifushezhi'></text>
					<view>佣金转余额</view>
				</view>
		...
		<uni-popup ref="popupRef">
			<view class="popup-tips">
				<div class="content">
					<view class="title">佣金转余额</view>
					<view class="input">
						￥<input v-model="money" placeholder="0.00" type="number" />
					</view>
					<view class="tips">
						注：输入金额不可大于当前佣金
					</view>
...
<script>
	...
	export default {
		...
		methods: {
			save() {
				let that = this
				if (Number(this.money) > Number(this.userInfo.total_brokerage_price)) {
			...
			showPopup() {
				if (Number(this.userInfo.total_brokerage_price) || true) {
	...
	.popup-tips {
		.content {
			...
			.btn {
				position: relative;
				top: 0;
				background-image: linear-gradient(180.00deg, rgba(67, 106, 188, 1), rgba(42, 55, 99, 1) 100%);
