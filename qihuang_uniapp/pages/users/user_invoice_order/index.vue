		...
		<view class='order-details'>
			...
			<view class='footer acea-row row-right row-middle'>
				<view class=" acea-row row-right row-middle">
					<view v-if="orderData.merchant && orderData.merchant.services_type == 0" class="bnt cancel" @click="goGoodCall">
						联系客服
					</view>
					<view v-else class="bnt cancel" @click="call">
						联系客服
					</view>
				</view>
			</view>
	...
	export default {
		...
		methods: {
			...
			goGoodCall() {
				let self = this
				uni.navigateTo({
					url: `/pages/chat/customer_list/chat?mer_id=${self.orderInfo.mer_id}&uid=${this.uid}&order_id=${self.orderInfo.order_id}`
				})
			},
			/**
			 * 拨打电话
