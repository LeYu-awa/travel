<template>
	<view>
		...
		<view class="content-wrap">
			...
			<!-- <topTips></topTips> -->
			<toptab v-if="isSee"></toptab>
			<!-- 热门板块 -->
			<popularSections :list="hotdata"></popularSections>
			<!-- 直播 -->
			<doctorLive :swiperList="liveswiper"></doctorLive>
			<networkDoctor :isSee="isSee" :swiperList="networkswiper" :swiperBottomList="swiperBottomList">
			</networkDoctor>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			async getHeadPageConfig() {
				const res = await getHeadPageConfig();
				this.isSee = res.data.is_see;
			},
			// #ifdef APP-PLUS
			appVersionConfig() {
				var that = this;
