	...
	<view :style="viewColor">
		<view class='productList'>
			...
			<block v-if="tabIndex == 1">
				<view class="list">
					<WaterfallsFlow typePage="video" :wfList='productList' @itemTap="godDetail" :type="1"
						@goShop="goShop" />
...
<script>
	...
	export default {
		...
		methods: {
			...
			godDetail(item) {
				uni.navigateTo({
					url: '/pages/video_card_goods/index?id=' + item.id
				});
			...
			bindDown(item, index) {
				this.firstKey = 0
				this.where.card_type = item.key
				this.where.page = 1
				this.where.is_hot = ''
				this.where.is_recommend = ''
				// if (index == 0) {
				// 	this.where.order = ''
				// } else if (index == 1) {
			...
			//查找产品
			async get_product_list(isPage) {
				let {
					data
				} = await getMemberCardList(this.where)
				data.list.filter(v => {
