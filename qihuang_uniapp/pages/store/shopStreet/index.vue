<template>
	<view>
		<view class='productList' :style="viewColor">
			<view class='search acea-row row-between-wrapper' :class="'styleType'+store_street_theme">
				...
				<view v-if="mer_location == 1" :class="'styleType'+store_street_theme" style="text-align: left;" class='iconfont search-right' @click='showMaoLocation(latitude,longitude)'>
					<view class="new-iconfont icon-dingwei"></view>
					<view class="right-text" v-if="recommend_address">
						{{recommend_address}}
					</view>
					<view class="iconfont icon-xiangyou" v-if="recommend_address"></view>
				</view>
				<view class='input acea-row row-between-wrapper'><text class='new-iconfont icon-sousuo'></text>
						...
						<view class="mer-item" v-for="(item,index) in storeList" :key='index'>
								...
								<view class="mer-name">
									...
									<view class="">
										<view class="mer-top">
											...
											<text v-else-if="item.is_trader" class="font-bg-red ml8">官方旗舰店</text>
										</view>
										<view class="mer-btn">
											<view class="">
												月售{{item.sales}}
							...
							<view class="pro-box" :class="'styleType'+store_street_theme">
								<navigator :url="`/pages/goods_details/index?id=${itemn.product_id}`" hover-class="none" class="pro-item" v-for="(itemn,indexn) in item.recommend" :key='indexn' v-if="item.recommend.length<=3">
									...
									<view class="store_name">{{itemn.store_name}}</view>
									<view class="price">
	...
	<customTab active="shopStreet" ></customTab>
