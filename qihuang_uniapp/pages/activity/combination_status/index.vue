<template>
	<view class="group-con" :style="viewColor">
		<view class="header acea-row row-between-wrapper">
			...
			<view class="text">
				...
				<view class="money">
					...
					<text class="team sColor" v-text="(combinationData.buying_count_num ? combinationData.buying_count_num : '') + '人拼'"></text>
				</view>
			</view>
		...
		<view class="wrapper">
			...
			<view class="tips t-color" v-else-if="pinkBool === -1">还差{{ count }}人，拼团失败</view>
			...
			<view class="list acea-row row-middle" :class="iShidden ? 'on' : 'result'">
				<view class="acea-row row-middle" v-if="pinkAll.length > 0">
					<view class="pictrue" v-for="(item, index) in pinkAll" :key="index"><image :src="item.avatar ? item.avatar : '/static/images/f.png'" /></view>
				</view>
				<view v-if="(pinkBool === 0 || pinkBool === -1)" class="pictrue" v-for="index in count" :key="index"><image class="img-none" src="../static/images/vacancy.png" /></view>
			</view>
			...
			<view class="teamBnt" v-if="pinkBool === 10 || pinkBool === -1" @click="goDetail(combinationData.product_group_id)">再次开团</view>
			<view class="cancel" @click="getCombinationRemove" v-if="pinkBool === 0 && userBool !== 0">
		...
		<view class="group-recommend">
			...
			<view class="list acea-row row-middle">
				<view class="item" v-for="(item, index) in storeCombinationHost" :key="index" @click="goDetail(item.product_group_id)">
					<view class="pictrue">
						<image :src="item.product.image" />
						<view class="team" v-text="item.buying_count_num + '人团'"></view>
					</view>
					<view class="name line1" v-text="item.product.store_name"></view>
					<view class="money" v-text="'￥' + item.price"></view>
				</view>
			</view>
		</view>
