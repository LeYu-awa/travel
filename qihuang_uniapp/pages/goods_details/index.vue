	...
	<view :style="viewColor">
		<view class="product-con">
							...
							<view v-if="storeInfo.atmosphere_pic" class='introduce skeleton-rect acea-row atmosphere'>
								...
								<view class="fenxiang_btn">
									<view @click="setCollect"><text class="iconfont"
									 ...
									 <!-- <view @click="appShare('WXSceneSession')"><text
					...
					<view class="store-wrapper" v-if="storeInfo.merchant && hide_mer_status !=1">
						<view class="store-hd skeleton-rect">
							<view v-if="storeInfo.merchant&&hide_mer_status != 1" class="store-info">
								<navigator :url="'/pages/store/home/index?id='+storeInfo.mer_id" hover-class="none"
									class="logo">
								...
								<navigator v-if="hide_mer_status != 1"
									:url="'/pages/store/home/index?id='+storeInfo.mer_id" class="link"
						...
						<view v-if="recommend && recommend.length > 0" class="con-box">
							<view class="acea-row row-between">
								...
								<navigator v-if="hide_mer_status!=1" class="moer-btn"
									:url="'/pages/store/home/index?id='+storeInfo.mer_id" hover-class="none">
			...
			<view class='footer acea-row row-between-wrapper' :class="{'footpl':hide_mer_status==1}">
				<navigator v-if="hide_mer_status !=1" :url="'/pages/store/home/index?id='+storeInfo.mer_id"
				...
				<view v-if="attr.productSelect.stock != 0" class='bnt acea-row skeleton-rect'>
					...
					<form @submit="goBuy" report-submit='true' v-if="attr.productSelect">
						<button class='buy bnts'
							:class="(storeInfo.type == 1 || storeInfo.type == 2 || storeInfo.mer_form_id) ? 'virtual_buy' : ''"
							form-type="submit" :disabled="attr.productSelect.stock == 0">立即购买</button>
					</form>
				</view>
				<view v-else class='bnt acea-row skeleton-rect'>
					<form report-submit='true'>
						<button v-if="procudt_increase_status == 1" class='buy bnts sold_out' form-type="submit"
			...
			<productWindow :attr="attr" :isShow='1' :iSplus='1' @myevent="onMyEvent" @ChangeAttr="ChangeAttr"
				...
				:payLimit="pay_limit" :maxCount="max_count" :minCount="min_count" :svipPrice="svipPrice"
			...
			<canvas class="canvas" type="2d" canvas-id='myCanvas'></canvas>
			<!-- 发送给朋友图片 -->
	...
	export default {
		...
		methods: {
			...
			async goPoster() {
				...
				console.log(that.share_pic)
				arr2 = [that.posterbackgd, that.storeImage, that.codeImg, that.share_pic];
				// #endif
				// #ifdef H5
