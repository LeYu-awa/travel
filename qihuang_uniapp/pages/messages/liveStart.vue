<template>
	<view class="content" v-if="list.length">
		<view class="text" v-for="item in list">
			...
			<view class="block">
				<view class="image">
					<image class="icon" src="/static/images/message/live_start_notice.png" mode="widthFix">
					</image>
				</view>
				<view class="item">
					...
					<view class="item-text">
						<text class="label">主播名称</text> <text>{{item.message_data_parsed.live_user_name}}</text>
					</view>
					<view class="item-text">
						<text class="label">主播标题</text> <text>{{item.message_data_parsed.live_title}}</text>
					</view>
					<view class="item-bottom" @click="$goNextPage('/pages/live_pusher/live_room/live_room?roomId=' + item.message_data_parsed.room_id)">
						<view class="btn">
							去观看直播
						</view>
					</view>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			this.pageQuery.message_type = 'live_start_notice'
			this.getList()
		},
		async onShow() {
			await markSystemMessageAsRead({
				message_type: 'live_start_notice'
			})
		},
		// 下拉刷新
		onPullDownRefresh() {
