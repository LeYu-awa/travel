	...
	<view class="swiperBg" :style="'padding-top:' + marginTop + 'rpx;background: linear-gradient(180deg, ' + bgColor0 + ' 50%, ' + bgColor1 + ' 100%);'">
		...
		<block v-if="swiperType == 0">
			<view class="item-img" v-for="(item, index) in imgUrls" :key="index" :style="{ padding: '0 ' + paddinglr + 'rpx', marginBottom: itemEdge + 'rpx' }">
				<image @click="goDetail(item)" :src="item.img"></image>
			</view>
		</block>
		<block v-else>
			<view
				class="swiper page_swiper"
				:class="[docConfig == 1 ? 'square' : docConfig == 0 ? 'circular' : 'nodoc', imgConfig ? '' : 'fillet', 'dot' + txtStyle]"
			...
			>
				<swiper
					...
					:style="{ height: (liveList.length / 2) * 320 + 500 + 'rpx' }"
					:current="swiperCur"
					@change="swiperChange"
				>
						...
						<swiper-item :class="{ active: index == swiperCur }">
							...
							<view class="live">
								<view class="live_title">
									<text class="live_title_left">直播专场</text>
									<text class="live_title_right">进入频道》</text>
								</view>
								<view class="live_list">
									<view class="live_list_item" v-for="(item, index) in liveList" :key="index" @click="goLiveRoom(item.id)">
										<image :src="item.room_image"></image>
										<view class="live_list_item_info">
											<view class="live_list_item_info_title">{{item.title}}</view>
										</view>
									</view>
								</view>
							</view>
...
export default {
	...
	methods: {
		...
		goLiveRoom(roomId){
			uni.navigateTo({
				url:'/pages/live_pusher/live_room/live_room?roomId=' + roomId
			})
		}
	}
};
