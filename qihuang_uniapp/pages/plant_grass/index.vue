<template>
	<view :style="viewColor">
		<view class="page_con">
			<view class="grass_header" :style="{'background-image': `url(${domain}/static/images/grass_header.png)`}">
				<view v-if="!navShow" class="header">
					<view class="navTab">
						<view v-if="community_status == 1" @click="goRelease"
							class="release_btn acea-row row-center-wrapper">
						...
						<navigator url="/pages/plantGrass/plant_search/index">
							<image src="/static/images/index/search.png" class="img-search"></image>
							<!-- <image class="avatar" :src="userInfo.avatar ? userInfo.avatar : '/static/images/f.png'">
							</image> -->
						</navigator>
					</view>
				<!-- 	<view class="search_count">
						<navigator url="/pages/plantGrass/plant_search/index" hover-class="none" class="search"><text
								class="iconfont icon-xiazai5"></text>搜索想看的文章</navigator>
				...
				<view class="longTab" v-if="tabActive == 1">
					<scroll-view scroll-x="true" style="white-space: nowrap; display: flex;" scroll-with-animation
						...
						<view class="longItem" :data-index="index" :class="index === tabClick ? 'click' : ''"
							v-for="(item, index) in menuList" :key="index" :id="'id' + index"
				...
				<view class="tab-cont" id="main" @touchmove="onTouchmove">
					...
					<view v-if="tabActive === 0">
						<view v-if="isLogin && followList.length > 0" class="follow_count"
							:class="(newData.status && newData.status.status && showTab) ? 'showFoot' : ''">
							<block v-for="(item, index) in followList" :key="index">
								<view class="list_count">
									<view class="title">
										<navigator hover-class="none"
											:url="'/pages/plantGrass/plant_user/index?id='+item.uid" v-if="item.author"
											class="author">
											<easy-loadimage class="picture" mode="widthFix"
												:image-src="item.author&&item.author.avatar || '/static/images/f.png'"></easy-loadimage>
											<text class="name">{{item.author.nickname || ''}}</text>
										</navigator>
										<view class="time">{{item.time}}</view>
									</view>
									<view class="product">
										<productConSwiper v-if="item.is_type == 1" :imgUrls="item.image">
										</productConSwiper>
										<view v-else class="videoSwiper" @click="goVideo(item)">
											<image class="image" :src="item.image[0]"></image>
											<image v-if="item.is_type == 2" class="video_img"
												src="../../static/images/stop.png"></image>
										</view>
									</view>
									<view class="pro_describle">
										<view class="mentioned" v-if="item.relevance && item.relevance.length > 0"
											@click="openMore(item)">
											<text class="title">查看TA提到的宝贝({{item.relevance.length}})</text>
											<view class="product_more">
												<view class="item">
													<easy-loadimage v-if="indexn<4"
														v-for="(itemn, indexn) in item.relevance" :key="indexn"
														mode="widthFix"
														:image-src="(itemn.spu && itemn.spu.image) || itemn.image"
														class="more_image"></easy-loadimage>
												</view>
										...
										<navigator v-if="item.topic" hover-class="none" class="product_cate"
											...
											<view>
												<text class="icon">#</text><text
													class="text">{{item.topic.topic_name}}</text>
											</view>
										</navigator>
										<view class="foot_bar">
											<!-- #ifdef MP -->
											<button class="time iconfont icon-fenxiang2" open-type="share"
												hover-class='none' @click="shareFriend(item)"></button>
											<!-- #endif -->
											<!-- #ifndef MP -->
											<button></button>
											<!-- #endif -->
											<view class="item">
												...
												<view v-if="community_reply_status == 1" class="item_count"
													@click="openCommon(item,index)">
					...
					<view v-show="tabActive === 1">
						<!-- 商品 -->
						<swiper :interval="interval" indicator-color="rgba(255,255,255,0.6)" :current="swiperCurrent"
							@change="swiperChange" :style="'height:'+swiperHeight+'px;'">
							<block v-for="(item,index) in menuList" :key="index">
								<swiper-item :item-id="`${item.category_id}`">
									<scroll-view @scroll="scrollLeft" scroll-y="true"
										:style="'height:'+swiperHeight+'px'" id="goods" refresher-enabled="true"
										:refresher-threshold="100" :refresher-triggered="triggeredDiscover"
										@refresherrefresh="onRefreshDiscover" @refresherpulling="onPulling"
										@refresherrestore="onRestore" @refresherabort="onAbort">
										<view
											v-if="cateGoods[item.category_id] && cateGoods[item.category_id].goods.length"
											class="goods-wrap" id="goods">
											<view class="goods">
												<WaterfallsFlow :isShow="false"
													:wfList='cateGoods[item.category_id].goods' :isFind="true"
													@likeToggle="likeToggle" @oncost="oncost" />
											</view>
										</view>
										<view
											:hidden="cateGoods[item.category_id] && !cateGoods[item.category_id].goodsLoading"
											class="acea-row row-center-wrapper loadingicon">
											<text
												class="iconfont icon-jiazai loading"></text>{{!cateGoods[item.category_id] ? 0 : cateGoods[item.category_id].goodsLoading}}
										</view>
										<view
											v-if="cateGoods[item.category_id] && cateGoods[item.category_id].goods.length == 0 && !cateGoods[item.category_id].goodsLoading"
											class="empty">
											<image :src="`${domain}/static/images/no_thing.png`"></image>
		...
		<!-- 评论弹窗 -->
		<comment ref="comment" :isShow="showComment" :userInfo="userInfo" :bottom="bottom" @successFul="commentSucces"
			@close="close"></comment>
		<!--自定义底部tab栏-->
		<customTab :newData="newData" :activeRouter="activeRouter"></customTab>
		<!-- 支付弹窗 -->
		<payShortVideo ref="paymentPop" :user_now_money="userInfo.now_money" :video_id="costData.community_id"
			:watch_cost="costData.watch_cost" @paySuccess='paySuccess'></payShortVideo>
		<!-- <uni-popup ref="costShow" type="center">
			<view class="costContent">
				<view class="cost-title">支付</view>
				<view class="cost-text">创作不易，您需支付{{costData.watch_cost}}元，观看视频内容 </view>
				<view class="cost-prc">{{costData.watch_cost}}元</view>
				<view class="cost-play">
					<radio-group @change="radioChange">
						<label class="uni-list-cell" v-for="(item, index) in costitems" :key="item.value">
							<view>
								<image :src="item.image" mode="widthFix"></image>{{item.name}}
							</view>
							<view>
				...
				<view class="cost-btn" @click="onpaly">去付款</view>
			</view>
		</uni-popup> -->
	</view>
</template>
<script>
	...
	import payShortVideo from '@/components/payShortVideo/index.vue';
	import {
		getNavigation
	} from '@/api/public.js';
		...
		data() {
			return {
				...
				costitems: [{
						value: '0',
						name: '微信支付',
						image: '/static/images/liveBroadcast/wecat.png'
					},
					{
						value: '1',
						name: '支付宝支付',
						image: '/static/images/liveBroadcast/zfb.png'
					}
				],
				videoList: [],
				costData: {},
			...
			uni.$on('startLike', function(data) {
				for (var i = 0; i < that.cateGoods[that.currentItemId]['goods'].length; i++) {
					...
					if (data.community_id == item['community_id']) {
						item['count_start'] = data.relevance_id ? item['count_start'] + 1 : item[
							'count_start'] - 1
		...
		methods: {
			...
			radioChange: function(evt) {
				for (let i = 0; i < this.costitems.length; i++) {
					if (this.costitems[i].value === evt.detail.value) {
						this.current = i;
						break;
					}
				}
			...
			oncost(data) {
				...
				this.$refs.paymentPop.open();
			},
			paySuccess(vidoId) {
				const index = this.videoList.findIndex(item => item.community_id == vidoId);
				if (index !== -1) {
					this.videoList[index].is_pay = 1;
				}
			},
			getNav() {
			...
			getGoods: function() {
				...
				graphicLstApi(data.where).then(res => {
					data.goodsLoading = false;
					data.goodsLoaded = res.data.length < that.where.limit;
					if (data.where.page == 1) {
						data.goods = [];
					}
					data.goods.push.apply(data.goods, res.data);
					data.where.page = data.where.page + 1;
					this.$set(this.cateGoods, cateId, {
						...data
					});
