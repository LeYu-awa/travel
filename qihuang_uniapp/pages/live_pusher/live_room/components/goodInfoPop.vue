<template>
    ...
    <view class="popUp" :style="{height:popHeight?'100vh':'68vh'}"> 
      <image @click="clickClose" src="/static/images/liveRoom/close.jpg" class="close-image"></image>
      <scroll-view scroll-y="true" class="scroll-Y" :show-scrollbar="false" @scroll="onscroll">
        <view class="popUp-content product-con">
...
var sysHeight = uni.getSystemInfoSync().statusBarHeight;
    ...
    watch: {
      show(newVal) {
        if (newVal) {
		  ...
		  this.id = this.$props.goodInfoData.product_id
    ...
    methods: {
      onscroll(e) {
        ...
        if (currentScrollTop < this.scrolldata) {
          // 如果当前滚动位置大于上一次的位置，说明是向下滑动
          if (e.detail.scrollTop==0&&this.popHeight) {
            this.popHeight = false
          }
        } else {
          // 否则，说明是向上滑动
          if (this.scrolldata==0) {
            this.popHeight = true
          }
        }
        this.scrolldata = currentScrollTop; // 更新上一次的滚动位置
      },
      clickClose() {
...
.scroll-Y{
  ...
  .ml8{
	...
	.product-con .footer .bnt .joinCart {
		...
		background-image: linear-gradient(90deg, #FEA10F 0%, #FA8013 100%);
		&.disabled{
			background: #bbb;
		}
	}
