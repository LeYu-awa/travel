	...
	<view class="wf-page">
		...
		<view>
			<view id="left" v-if="leftList.length">
				<view v-for="(item,index) in leftList" :key="index" class="wf-item" @tap="itemTap(item, item.key)">
					<WaterfallsFlowItem :item="item" :isAuth="isAuth" :uid="uid" :isShow="isShow" :tab="tab"
						@likeToggle="(item)=>likeToggle(false, index, item)" @oncost="oncost(item)" />
				</view>
			</view>
		</view>
		<!--right-->
		<view>
			<view id="right" v-if="rightList.length">
				<view v-for="(item,index) in rightList" :key="index" class="wf-item" @tap="itemTap(item, item.key)">
					<WaterfallsFlowItem :item="item" :isAuth="isAuth" :uid="uid" :isShow="isShow" :tab="tab"
						@likeToggle="(item)=>likeToggle(true, index, item)" @oncost="oncost(item)" />
				</view>
			</view>
		</view>
	...
	export default {
					...
					if (this.wfList.length) {
						...
						this.allList.forEach((v, i) => {
							if (this.allList.length < 3 || (this.allList.length <= 7 && this.allList.length - i >
								...
								if (i % 2) {
									this.rightList.push({...v, key: i});
								} else {
									this.leftList.push({...v, key: i});
								}
							}
						});
		...
		methods: {
			...
			waterFall() {
				...
				if (i == 0) {
					// 初始化，从左边开始插入
					this.leftList.push({...this.allList[i], key: i});
					// 更新左边列表高度
					this.getViewHeight(0);
				} else if (i == 1) {
					// 第二个item插入，默认为右边插入
					this.rightList.push({...this.allList[i], key: i});
					// 更新右边列表高度
			...
			// item点击
			itemTap(item, index) {
				this.$emit('itemTap', item, index)
			},
			oncost(item) {
				this.$emit('oncost', item)
			},
