<template>
	<view class="easy-loadimage" :id="uid">
		<image class="origin-img" :src="imageSrc" mode="aspectFill" v-if="loadImg&&!isLoadError" v-show="showImg"
			:class="{'no-transition':!openTransition,'show-transition':showTransition&&openTransition}" lazy-load
			@load="handleImgLoad" @error="handleImgError">
		</image>
		<view class="loadfail-img" v-else-if="isLoadError"
			:style="{ 'background-image': `url(${domain}/static/images/loadfail.png)`}"></view>
		<view :class="['loading-img','spin-circle',loadingMode]" v-show="!showImg&&!isLoadError"
			:style="{ 'background-image': `url(${domain}/static/images/loading.gif)`}"></view>
	</view>
</template>
<script>
	import {
			...
			return {
				...
				scrollFn: throttle(function() {
					...
					query.select('#' + id).boundingClientRect(data => {
						...
						if (!data) return;
						if (data.top - that.viewHeight < 0 || true) {
